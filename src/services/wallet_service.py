...
from src.db.wallet_functions import (
    create_wallet, get_wallet_by_user_id, get_wallet_balance,
    get_wallet_id_by_user_id, reserve_tokens_fifo, confirm_debit, release_reservation,
    get_transaction_history,
    get_active_subscription, get_subscription_history, create_pending_subscription,
    confirm_token_batch, fail_token_batch, check_existing_token_batch, get_token_batch_by_payment_id,
    update_subscription_metadata, process_subscription_charge, get_subscription_by_user_id,
    get_subscription_by_subscription_id,cancel_future_subscription_batches
)
from src.services.payment_service import PaymentService
from src.resources.schemas.wallet import SubscriptionWebhookEvent
from src.db.database import User, TokenBatch, TokenTransaction, SubscriptionInterval
from src.db.wallet_functions import create_token_batch
from src.db.wallet_functions import get_user_email
from src.db.enums import SubscriptionTier, TransactionSource, TransactionStatus, SubscriptionStatus, TransactionType
from src.config.constants import (
    CASPR_PAYMENT_API_KEY, CASPR_PAYMENT_BASE_URL, SIGNUP_BONUS_TOKENS, REPORT_GENERATION_COST,
    SUBSCRIPTION_INFO, TAX_INFO, TOKEN_PRICE_INFO,TOKEN_EXPIRY_DAYS,
...
class WalletService:
    ...
    async def initialize_wallet(
        ...
        try:
            ...
            if bonus_tokens > 0:
                from src.db.referral_functions import credit_referral_bonus
            ...
            logger.critical(
                f"[WALLET_INIT_EXCEPTION] user_id={user_id}, error={str(e)}", 
                exc_info=True
            )
    ...
    @staticmethod
    async def has_sufficient_balance(user_id: str, tokens_needed: int, session: AsyncSession) -> Dict[str, Any]:
    ...
    @staticmethod
    async def release_report_reservation(report_id: str, session: AsyncSession, reason: str = "generation_failed") -> Dict[str, Any]:
    ...
    @staticmethod
    async def get_subscription_history(user_id: str, session: AsyncSession, limit: int = 10) -> Dict[str, Any]:
    ...
    async def create_pending_subscription(
        ...
        logger.info(f"Creating pending subscription: user_id={user_id}, tier={tier}, external_id={subscription_id}")
    ...
    async def create_subscription_order(
        ...
        logger.info(f"Creating subscription (via PaymentService): plan={plan_tier}, duration={plan_duration}")
    ...
    async def change_subscription_plan(
        ...
        """
        from src.services.subscription_update_validator import (
        ...
        try:
            ...
            if not active_subscription.get("success") or not active_subscription.get("subscription"):
            ...
            token_usage_result = await calculate_fair_token_usage(
                external_sub_id,
                user_id,
                session
            )
            ...
            change_record = {
                ...
                "days_elapsed": token_usage_result.get("days_elapsed", 0),  # NEW: For same-day detection
                "actual_consumed_tokens": token_usage_result.get("actual_consumed_tokens", 0)  # NEW: For same-day logic
            }
            ...
            from src.db.wallet_functions import update_subscription_metadata
            ...
            if not payment_result.get("success"):
                ...
                return {
                    "success": False,
                    "error": payment_result.get("error", "Failed to update subscription at payment gateway"),
                    "status_code": 500
                }
            ...
            logger.critical(
                f"[SUBSCRIBE_CHANGE_PLAN_EXCEPTION] user_id={user_id}, error={str(e)}", 
                exc_info=True
            )
            return {
                "success": False,
    ...
    async def handle_subscription_updated_webhook(
        ...
        if not subscription:
            ...
            return {"success": False, "error": f"Subscription record not found for ID: {subscription_id}"}
        ...
        if not old_tier or not new_tier:
            ...
            return {
                "success": False,
                "error": "Invalid plan change data: missing tier information"
            }
        ...
        from src.db.wallet_functions import _sync_wallet_balances
        ...
        from src.db.wallet_functions import create_plan_change_token_batches
        ...
        if not batch_result.get("success"):
            ...
            return {"success": False, "error": f"Failed to create token batches: {batch_result.get('error')}"}
        ...
        for i, batch_id in enumerate(batch_ids):
            ...
            if is_first_batch:
                ...
                if days_elapsed == 0:
                    ...
                    description_parts.append(f"(Same-day upgrade)")
                    if actual_consumed_tokens > 0:
                        description_parts.append(f"(-{actual_consumed_tokens:,} consumed from old plan)")
                    ...
                    if over_under_consumption != 0:
                        if over_under_consumption < 0:
                            ...
                            description_parts.append(f"(+{abs(over_under_consumption):,} fair-use credit)")
                        else:
                            # Over-consumed: tokens deducted
                            description_parts.append(f"(-{over_under_consumption:,} fair-use deduction)")
                
                description = " - ".join(description_parts)
            else:
                # Remaining batches: standard unlock (monthly for yearly plans)
                batch_label = "Monthly Unlock" if new_duration == "yearly" else "Subscription Renewal"
        ...
        if event_data.current_start and event_data.current_end:
            interval_start = parse(event_data.current_start) if isinstance(event_data.current_start, str) else event_data.current_start
            interval_end = parse(event_data.current_end) if isinstance(event_data.current_end, str) else event_data.current_end
        else:
        ...
        if not interval_payment_id:
            interval_payment_id = f"pc_{subscription_id[-20:]}_{int(datetime.now(timezone.utc).timestamp())}"[:50]
        ...
        latest_change["changed_at"] = datetime.now(timezone.utc).isoformat()
        plan_change_history[-1] = latest_change
        
        # Next payment for yearly = end of current period (charge_at or current_end)
        next_payment = getattr(event_data, "charge_at", None) or event_data.current_end or getattr(event_data, "next_payment_datetime", None)
        ...
        update_metadata = {
            ...
            "gateway": event_data.gateway,
            "plan_change_completed_at": datetime.now(timezone.utc).isoformat(),
    ...
    async def handle_subscription_upgrade(
        ...
        try:
            ...
            logger.info(
                f"[SUBSCRIPTION_UPGRADE_CANCELLING_FUTURE] user_id={user_id}"
            )
            ...
            logger.critical(
                ...
                f"upgrade_path={old_tier} -> {new_plan_tier}, error={str(e)}",
                exc_info=True
            )
            try:
                await session.rollback()
    ...
    async def create_subscription(
        ...
        try:
            ...
            active_subscription = await get_active_subscription(session, user_id=user_id, filter_created_status=True)
            ...
            if active_subscription.get("success") and active_subscription.get("subscription") and active_subscription.get("subscription").get("status") not in (SubscriptionStatus.CANCELLED.value, SubscriptionStatus.EXPIRED.value, SubscriptionStatus.COMPLETED.value):
                ...
                if sub_status in (SubscriptionStatus.ACTIVE.value, SubscriptionStatus.PENDING.value) and has_active_interval:
                    logger.warning(
                ...
                elif sub_status in (SubscriptionStatus.ACTIVE.value, SubscriptionStatus.PENDING.value) and not has_active_interval:
                    logger.info(
                    ...
                    await update_subscription_metadata(
                        active_subscription.get("subscription").get("id").strip() if active_subscription.get("subscription").get("id") else "",
                        {},
                        session,
            ...
            logger.error(
                f"[SUBSCRIBE_CREATE_VALUE_ERROR] user_id={user_id}, error={str(e)}", 
                exc_info=True
            )
            return {
                "success": False,
            ...
            logger.critical(
                f"[SUBSCRIBE_CREATE_EXCEPTION] user_id={user_id}, error={str(e)}", 
                exc_info=True
            )
            return {
                "success": False,
    ...
    async def cancel_subscription(
        ...
        try:
            ...
            cancelled_at_iso = datetime.now(timezone.utc).isoformat()
            
            # Get cancel_at from payment service response
            payment_cancel_data = cancel_result.get("data", {}) if cancel_result else {}
            cancel_at_future = payment_cancel_data.get("cancel_at")  # Future datetime from payment service
            ...
            logger.critical(
                f"[SUBSCRIPTION_CANCEL_EXCEPTION] user_id={user_id}, "
                f"error={str(e)}",
                exc_info=True
            )
            return {
    ...
    async def payment_webhook_handler(
        ...
        try:
            ...
            if not status or not isinstance(status, str):
                logger.error(f"[PAYMENT_WEBHOOK_INVALID_STATUS] payment_id={payment_id}, status={status}")
    ...
    async def handle_subscription_webhook(
        event_data: SubscriptionWebhookEvent,
        session: AsyncSession
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"[SUBSCRIPTION_WEBHOOK_HANDLER_START] event={event_type}, "
        ...
        if not event_type or not sub_id_ext:
            ...
            return {"success": False, "error": "Invalid webhook data: missing event or subscription_id"}
        
        try:
            
            # 1. Handle METADATA update events (Legacy or Specialized)
            if event_type == "SUBSCRIPTION_ACTIVATED":
                return await update_subscription_metadata(sub_id_ext, {}, session, SubscriptionStatus.ACTIVE)
            ...
            # 2. Handle CHARGE events (Credit tokens)
                ...
                if not subscription:
                    ...
                    return {"success": False, "error": f"Subscription record not found for ID: {sub_id_ext}"}
                ...
                start_date = parse(event_data.start_date) if event_data.start_date else datetime.now(timezone.utc)
                end_date = parse(event_data.end_date) if event_data.end_date else start_date + timedelta(days=30)
                ...
                result = await process_subscription_charge(
                    ...
                    tokens_per_month=tokens_per_month,
                    session=session,
                    payment_id=getattr(event_data, 'payment_id', None) or f"charge_{sub_id_ext}_{event_data.paid_count}",
                    metadata={
                        "amount": amount,
                        "currency": currency,
                ...
                active_subscription = await get_active_subscription(session, subscription_id=sub_id_ext)
                if not active_subscription.get("success") or not active_subscription.get("current_interval"):
                    return await update_subscription_metadata(
                        sub_id_ext, 
                        {"error_code": event_data.error_code, "error_message": event_data.error_message}, 
                        session,
            ...
            logger.error(
                ...
                f"subscription_id={sub_id_ext}, error={str(e)}", 
                exc_info=True
            )
            ...
            logger.error(
                ...
                f"subscription_id={sub_id_ext}, missing_key={str(e)}", 
                exc_info=True
            )
        ...
        try:
            ...
            if existing_batch:
                ...
                if status in [TransactionStatus.COMPLETED, TransactionStatus.FAILED]:
                    ...
                    razorpay_order = razorpay_result.get("data") if razorpay_result.get("success") else None
                    razorpay_status = razorpay_order.get("status") if razorpay_order else None

                    if razorpay_status in ('COMPLETED','FAILED','CANCELLED'):
                        wallet_result = await WalletService.payment_webhook_handler(payment_id, session, razorpay_status)
            ...
            logger.error(
                f"[TOPUP_ORDER_VALUE_ERROR] user_id={user_id}, error={str(e)}", 
                exc_info=True
            )
            return {"success": False, "error": f"Invalid parameter: {str(e)}"}
        except KeyError as e:
            logger.error(
                f"[TOPUP_ORDER_KEY_ERROR] user_id={user_id}, missing_key={str(e)}", 
                exc_info=True
            )
            return {"success": False, "error": f"Missing required field: {str(e)}"}
        except Exception as e:
            logger.critical(
                f"[TOPUP_ORDER_EXCEPTION] user_id={user_id}, error={str(e)}", 
                exc_info=True
            )
            return {"success": False, "error": "Failed to create topup order"}

    @staticmethod
    async def process_topup(user_id: str, tokens: int, payment_id: str, amount: float, session: AsyncSession) -> Dict[str, Any]:
    ...
    async def process_topup_confirmation(
        ...
        if result.get("success"):
            ...
            logger.error(
                f"[TOPUP_CONFIRMATION_FAILED] payment_id={payment_id}, "
    ...
    async def process_topup_failure(
        ...
        if result.get("success"):
            logger.info(
                f"[TOPUP_FAILURE_SUCCESS] payment_id={payment_id}, "
                f"marked as failed"
            )
            return {
                "success": True,
            ...
            logger.error(
                f"[TOPUP_FAILURE_FAILED] payment_id={payment_id}, "
                f"error={result.get('error')}"
            )
            return result
    ...
    async def get_payment_status(
        ...
        try:
            ...
            if not token_batch:
                logger.warning(f"[GET_PAYMENT_STATUS_NOT_FOUND] payment_id={payment_id}, user_id={user_id}")
                return {
                    "success": False,
            ...
            if token_batch.status == TransactionStatus.PENDING:
                ...
                if razorpay_status in ('COMPLETED','FAILED','CANCELLED'):
                    wallet_result = await WalletService.payment_webhook_handler(payment_id, session, razorpay_status, razorpay_order.get("amount"), razorpay_order.get("currency"), razorpay_order.get("payment_method"), razorpay_order.get("payment_method_details"))
            ...
            return {
                ...
                "created_at": token_batch.created_at.isoformat() if token_batch.created_at else None,
                "expires_at": token_batch.expires_at.isoformat() if token_batch.expires_at else None
            }
    ...
    def calculate_subscription_tax(
        ...
        if not plan_data:
            return {"success": False, "error": f"Plan '{plan_name}' or type '{plan_type}' not found"}
    ...
    async def update_payment_method(
        ...
        try:
            ...
            if not payment_service_url or not api_key:
                logger.error("[UPDATE_PAYMENT_METHOD_CONFIG_ERROR] Payment service configuration missing")
                return {
                    "success": False,
            ...
            plan_tier = subscription.meta_data.get("plan_tier", "PRO") if subscription.meta_data else "PRO"
            plan_duration = (subscription.meta_data.get("plan_duration") or "monthly").lower() if subscription.meta_data else "monthly"
            country = subscription.meta_data.get("country", "US") if subscription.meta_data else "US"
            ...
            return {
                "success": False,
                "error": f"Payment service communication error: {str(e)}"
            }
        except Exception as e:
            logger.error(
                f"[UPDATE_PAYMENT_METHOD_EXCEPTION] subscription_id={subscription_id}, "
                f"error={str(e)}",
                exc_info=True
            )
            return {
