...
class Card(Base):
    ...
    citations = Column(
        JSONB, nullable=True, comment="Citations or references specific to this card"
    )
    ...
    updated_at = Column(
        ...
        nullable=False,
        comment="Timestamp when the card was last modified",
    )
    summary = Column(Text, nullable=True, comment="Summary of the card content")
    type = Column(
        String(50), nullable=True, comment="Type of the card (title/subtitle/toc/section/viz)"
    )
    ...
    is_deleted = Column(
        Boolean, nullable=False, default=False, comment="Whether the card is deleted"
    )
    changed_since_es = Column(
        Boolean,
        nullable=False,
        default=False,
        comment="Whether card changed since last ES generation",
    )
    last_es_version_used = Column(
        Integer, nullable=True, comment="ES version number this card summary was used in"
    )
...
class CardVersion(Base):
    ...
    parent_card_id = Column(
        ...
        nullable=False,
        comment="Foreign key to cards.id (primary key)",
    )
    section_id = Column(
        CHAR(36),
        nullable=True,
        comment="Business card_id from cards.card_id - tracks which logical card was modified",
    )
    ...
    refinement_type = Column(
        String(50), nullable=True, comment="Refinement type section,subsection or visualization"
    )
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
...
class Table(Base):
    ...
    base64_s3_uri = Column(
        Text, nullable=True, comment="S3 URI for the visualization image converted from base64"
    )
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
