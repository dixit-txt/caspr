...
async def insert_table(
    session: AsyncSession, card_id: str, table_data: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    if not card_id:
        ...
        return {"success": False, "error": "Missing required field: card_id"}
    ...
    try:
        ...
        parent_stmt = select(Card.id).where(Card.card_id == card_id, Card.is_active == True)
        ...
        logger.error(f"Database error creating table: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error creating table: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Unexpected error: {e!s}"}


async def insert_card(
    session: AsyncSession, report_id: str, report_card: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if existing_cards:
            logger.info(
                f"Deactivating {len(existing_cards)} previous versions of card_id: {card_id}"
            )
        ...
        if "created_at" not in report_card or not report_card["created_at"]:
            report_card["created_at"] = datetime.now(UTC)
        ...
        logger.info(
            f"Successfully created card with ID: {new_card_id}, card_id: {card_id}, version: {version}"
        )
        return {
            "success": True,
        ...
        logger.error(f"Database error creating card: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error creating card: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def get_tables_for_card(card_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Successfully retrieved {len(tables_data)} tables for card with business card_id: {card_id}"
        )
        return {"success": True, "tables": tables_data}

    except SQLAlchemyError as e:
        logger.error(f"Database error retrieving card tables: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error retrieving card tables: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}


async def get_report_cards(
    report_id: str, session: AsyncSession, version_id: str = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for card in cards:
            ...
            if isinstance(card.content, dict):
                ...
                content_tables = _normalize_tables_for_response(
                    card.content.get("tables", tables_data)
                )
            else:
            ...
            card_data = {
                ...
                "type": card.type,
                "section": [
                    {"name": content_name, "content": content_text, "tables": content_tables}
                ],
        ...
        logger.info(
            f"Successfully retrieved {len(cards_data)} cards for report with ID: {report_id}"
        )
        return {"success": True, "cards": cards_data}

    except SQLAlchemyError as e:
        logger.error(f"Database error retrieving report cards: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error retrieving report cards: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def get_table_id_markdown_map(report_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        stmt = (
            select(Table)
            .join(Card, Table.parent_card_id == Card.id)
            .where(Table.report_id == report_id, Card.is_active == True)
        )
        result = await session.execute(stmt)
        ...
        logger.info(
            f"Successfully retrieved {len(table_id_markdown_map)} tables from active cards for report: {report_id}"
        )
        return {"success": True, "table_id_markdown_map": table_id_markdown_map}
    except Exception as e:
        logger.error(f"Unexpected error retrieving table_id_markdown_map: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}
    except SQLAlchemyError as e:
        logger.error(f"Database error retrieving table_id_markdown_map: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
...
async def get_report_in_cards_format(report_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Successfully retrieved {len(formatted_cards)} active cards for report with ID: {report_id}"
        )
...
async def insert_card_version(
    ...
    """
    logger.info(
        f"Creating card version record for card_id (PK): {card_id}, section_id: {section_id}, type: {refinement_type}, subsection_id: {subsection_id}"
    )
    ...
    try:
        ...
        logger.error(f"Database error creating card version: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error creating card version: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Unexpected error: {e!s}"}


async def refine_card_in_db(
    session: AsyncSession,
    report_id: str,
    ...
    """
    logger.info(
        f"Refining card for report: {report_id}, type: {refinement_type}, subsection_id: {subsection_id}"
    )
    ...
    try:
        ...
        if not business_card_id:
            ...
            return {"success": False, "error": "No business card_id found in card data"}

        # Deactivate all previous versions of this card
        existing_cards_stmt = select(Card).where(
            Card.card_id == business_card_id, Card.report_id == report_id
        )
        existing_cards_result = await session.execute(existing_cards_stmt)
        existing_cards = existing_cards_result.scalars().all()
        ...
        if existing_cards:
            ...
            logger.info(
                f"Deactivating {len(existing_cards)} previous versions of card_id: {business_card_id}"
            )
            for existing_card in existing_cards:
                existing_card.is_active = False
            await session.flush()
        ...
        if not table_insert_result.get("success"):
            logger.warning(
                f"Failed to insert tables from updated card: {table_insert_result.get('error')}"
            )
        else:
            logger.info(
                f"Successfully inserted {table_insert_result.get('total_processed', 0)} tables from updated card"
            )
        ...
        logger.error(f"Database error refining card: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error refining card: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def insert_tables_from_updated_card(
    ...
    """
    logger.info(
        f"Extracting and inserting tables from updated card for business_card_id: {business_card_id}"
    )

    if not updated_card:
        logger.warning("No updated card data provided")
        return {"success": True, "inserted_tables": []}
    ...
    try:
                    ...
                    for idx, table in enumerate(section["tables"]):
                        ...
                        if table_id:
                            ...
                            if insert_result.get("success"):
                                ...
                                logger.info(
                                    f"Successfully inserted section table with ID: {table_id}"
                                )
                            else:
                                logger.warning(
                                    f"Failed to insert section table {table_id}: {insert_result.get('error')}"
                                )
                        else:
                            logger.warning("Section table ID is empty or None")
                    ...
                    for idx, table in enumerate(subsection["tables"]):
                        ...
                        if table_id:
                            ...
                            if insert_result.get("success"):
                                inserted_table_ids.append(insert_result.get("table_id"))
                                logger.info(
                                    f"Successfully inserted subsection table with table_ID: {table_id}"
                                )
                            else:
                                logger.warning(
                                    f"Failed to insert subsection table {table_id}: {insert_result.get('error')}"
                                )
                        else:
                            logger.warning("Subsection table ID is empty or None")

        logger.info(
            f"Successfully processed tables for card {business_card_id}, inserted {len(inserted_table_ids)} tables"
        )
        return {
            "success": True,
        ...
        logger.error(f"Error processing tables from updated card: {e!s}", exc_info=True)
        return {
            "success": False,
            "error": f"Error processing tables: {e!s}",
            "inserted_tables": inserted_table_ids,
        }


async def delete_card_or_subsection(session, card_id: str, subsection_id: str | None, toc: str):
    """
    ...
    try:
        ...
        card_query = select(Card).where(
            Card.card_id == card_id, Card.is_active == True, Card.is_deleted == False
        )
        result = await session.execute(card_query)
        active_card = result.scalar_one_or_none()

        if not active_card:
            return {"success": False, "error": f"No active card found with card_id: {card_id}"}
        ...
        if subsection_id is None:
            ...
            if not current_sub_sections:
                return {"success": False, "error": f"No subsections found in card {card_id}"}
            ...
            try:
                ...
                logger.info(
                    f"Successfully regenerated summary after subsection deletion for card {card_id}"
                )
            except Exception as e:
                logger.error(f"Error regenerating summary after subsection deletion: {e!s}")
        ...
        return {"success": False, "error": f"Failed to delete card/subsection: {e!s}"}
...
async def delete_visualization(
    ...
    try:
        ...
        table_query = (
            select(Table)
            .where(Table.report_id == report_id, Table.table_id == table_id)
        ...
        if table.parent_card_id != active_card.id:
            logger.info(
                f"Updating table {table_id} parent_card_id from {table.parent_card_id} to {active_card.id} (card was refined)"
            )
        ...
        if subsection_id:
            ...
            if not active_card.sub_sections:
                logger.error(
                    f"Card has no subsections, but subsection_id {subsection_id} was provided"
                )
                await session.rollback()
                return {
                    "success": False,
                ...
                if isinstance(subsection, dict) and subsection.get("id") == subsection_id:
                    ...
                    for table_entry in subsection.get("tables", []):
                        if table_entry.get("table_id") == table_id:
                            ...
                            logger.info(
                                f"Cleared visualization in subsection {subsection_id} for table {table_id}"
                            )
                            break
                    break
        ...
        active_card.updated_at = datetime.now(UTC)
        ...
        return {
            "success": False,
            "error": f"Failed to delete visualization: {e!s}",
            "error_type": "server_error",
        }


async def revert_card_to_version(
    session: AsyncSession, report_id: str, card_id: str
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if current_version == 1:
            return {"success": False, "error": "Cannot revert version 1 - it's the first version"}
        ...
        logger.info(
            f"Reverted card {card_id} from version {current_version} to version {previous_card.version}"
        )
        ...
        await session.rollback()
        logger.error(f"Database error reverting card: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        await session.rollback()
        logger.error(f"Unexpected error reverting card: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def get_latest_card_version(session: AsyncSession, card_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Database error getting latest card version: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error getting latest card version: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}


async def get_cards_for_version(
    report_id: str, version_id: str, session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        return {"success": True, "cards": cards_data, "from_version_snapshot": True}

    except Exception as e:
        logger.error(f"Error getting cards for version: {e!s}", exc_info=True)
...
async def get_refinement_history(report_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        if refinement_record is None:
            ...
            return {"success": True, "refine_history": []}

        refine_history = refinement_record.refine_history or []
        logger.info(
            f"Found refinement history with {len(refine_history)} entries for report_id: {report_id}"
        )
        return {"success": True, "refine_history": refine_history}

    except SQLAlchemyError as e:
        logger.error(f"Database error fetching refinement history: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error fetching refinement history: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}


async def update_refinement_history(
    report_id: str, refine_history: list, session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    """
    logger.info(
        f"Updating refinement history for report_id: {report_id} with {len(refine_history)} entries"
    )
    ...
    try:
        ...
        if refinement_record is None:
            logger.warning(
                f"No refinement history record found for report_id: {report_id}, creating one"
            )
            refinement_record = RefinementHistory(
                id=str(uuid7()), report_id=report_id, refine_history=refine_history
            )
        ...
        logger.error(f"Database error updating refinement history: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error updating refinement history: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}
