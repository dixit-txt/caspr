...
2. **Citation URL format** – Every cited URL must follow this exact format:
   ...
   - **Preserve inline citations:** Links already in `[Source Name](url)` format (a real http(s) URL from web search) MUST be kept exactly as-is. Do NOT remove valid citation links the model included, and do NOT renumber or relabel them. If a citation label is a bare number, replace that label with the source's name; never introduce a numbered label.
