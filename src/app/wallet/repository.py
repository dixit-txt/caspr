...
from src.db.database import Wallet, Subscription, SubscriptionInterval, TokenBatch, TokenTransaction
from src.db.enums import (
    SubscriptionTier, TransactionType, TransactionSource, TransactionStatus, SubscriptionStatus
)
from src.config.constants import (
    SIGNUP_BONUS_TOKENS, TOKEN_EXPIRY_DAYS,
    TIER_COST_USD, X_API_KEY
)
...
async def create_wallet(user_id: str, session: AsyncSession, tier: str = SubscriptionTier.FREE.value) -> Dict[str, Any]:
    ...
    try:
        ...
        wallet = Wallet(
            id=wallet_id,
            user_id=user_id,
            available_balance=SIGNUP_BONUS_TOKENS,
            reserved_balance=0
        )
        session.add(wallet)
        await session.flush()
        ...
        logger.error(f"Database error creating wallet: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(f"Unexpected error creating wallet: {str(e)}", exc_info=True)
        await session.rollback()
    ...
    try:
        ...
        return {
            ...
            "wallet": {
                ...
                "subscription_tier": subscription.current_tier.value if subscription else SubscriptionTier.FREE.value,
                "subscription_status": subscription.status.value if subscription else None
            }
        }
    except SQLAlchemyError as e:
        logger.error(f"Database error getting wallet: {str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
...
async def get_wallet_balance(
    ...
    try:
        ...
        logger.error(f"Database error getting balance: {str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}


async def get_wallet_id_by_user_id(user_id: str, session: AsyncSession) -> Optional[str]:
    """Get wallet ID for a user."""
    result = await session.execute(select(Wallet.id).where(Wallet.user_id == user_id))
    return result.scalar_one_or_none()


async def get_user_email(user_id: str, session: AsyncSession) -> Optional[str]:
...
async def get_or_create_subscription(
    user_id: str,
    session: AsyncSession,
    subscription_id_ext: Optional[str] = None
) -> Subscription:
...
async def get_subscription_by_user_id(user_id: str, session: AsyncSession) -> Optional[Subscription]:
    """Plain getter for subscription object."""
    result = await session.execute(select(Subscription).where(Subscription.user_id == user_id).order_by(Subscription.created_at.desc()))
    return result.scalars().all()


async def get_subscription_by_subscription_id(subscription_id: str, session: AsyncSession) -> Optional[Subscription]:
    """Plain getter for subscription object."""
...
async def create_subscription_interval(
    ...
    sub_metadata: Optional[Dict[str, Any]] = None,
    ...
    try:
        ...
        for i in range(splits):
            batch_start_at = now + relativedelta(months=i)  # Proper monthly calculation accounting for varying month durations
            ...
            token_batch = TokenBatch(
                ...
                currency=metadata.get("currency") if metadata and metadata.get("currency") else None,
        ...
        for batch_info in batch_info_list:
            transaction = TokenTransaction(
                ...
                meta_data={"subscription_id": subscription.id, "tier": tier, "payment_id": payment_id, "is_yearly": is_yearly}
            )
            session.add(transaction)
        ...
        logger.error(f"Database error creating subscription interval: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {str(e)}"}


async def get_active_subscription(session: AsyncSession, user_id: Optional[str] = None, subscription_id: Optional[str] = None, filter_created_status: Optional[bool] = False) -> Dict[str, Any]:
    ...
    try:
        ...
        for sub in subscriptions:
            ...
            interval_result = await session.execute(
                select(SubscriptionInterval).where(
            ...
            if len(current_intervals) > 1:
                logger.error(f"Multiple active intervals found for user {user_id} and subscription {sub.id}")
        ...
        logger.error(f"Database error getting subscription: {str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
...
async def get_subscription_history(user_id: str, session: AsyncSession, limit: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Database error getting subscription history: {str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
...
async def create_pending_subscription(
    ...
    logger.info(f"Creating pending subscription for user_id: {user_id}, tier: {tier}, external_id: {subscription_id}")
    ...
    try:
        ...
        logger.error(f"Database error creating pending subscription: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(f"Unexpected error creating pending subscription: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": str(e)}
...
async def update_subscription_metadata(
    ...
    try:
        result = await session.execute(
            select(Subscription).where(Subscription.subscription_id == subscription_id_ext).with_for_update()
        )
        subscription = result.scalar_one_or_none()
        ...
        logger.error(f"Error updating subscription metadata: {str(e)}", exc_info=True)
        return {"success": False, "error": str(e)}
...
async def process_subscription_charge(
    ...
    try:
        ...
        logger.error(f"Error processing subscription charge: {str(e)}", exc_info=True)
        return {"success": False, "error": str(e)}
...
async def cancel_future_subscription_batches(
    ...
    """
    logger.info(
        f"[CANCEL_FUTURE_BATCHES] wallet_id={wallet_id}, subscription_id={subscription_id}"
    )
    ...
    try:
        ...
        result = await session.execute(
            select(TokenBatch).where(
                ...
                TokenBatch.meta_data['subscription_id'].astext == subscription_id  # Match subscription
            ).with_for_update()
        )
        ...
        logger.error(
            ...
            f"subscription_id={subscription_id}, error={str(e)}",
            exc_info=True
        )
        return {
            "success": False,
...
async def invalidate_subscription_batches_and_get_reserved(
    ...
    try:
        ...
        result = await session.execute(
            select(TokenBatch).where(*batch_filters).with_for_update()
        )
        ...
        if invalidated_count > 0 and source_duration == "yearly":
            from src.db.database import TokenTransaction
...
async def create_plan_change_token_batches(
    ...
    try:
        ...
        if days_elapsed == 0:
            ...
            first_batch_initial_tokens = tokens_per_month - over_under_consumption + available_tokens
            logger.info(
        ...
        first_batch_initial_tokens = max(0, first_batch_initial_tokens)  # Ensure non-negative (safety check)
        ...
        for i in range(num_batches):
            ...
            token_batch = TokenBatch(
                ...
                amount=metadata.get("amount") if metadata else None,  # Amount difference for plan change
                currency=metadata.get("currency") if metadata else None,  # Currency from subscription
                source_type=TransactionSource.SUBSCRIPTION,
                payment_id=payment_id,
                start_at=batch_start_at,
        ...
        logger.error(
            f"[CREATE_PLAN_CHANGE_BATCHES_ERROR] wallet_id={wallet_id}, "
            f"error={str(e)}",
            exc_info=True
        )
        return {
        ...
        logger.error(
            f"[CREATE_PLAN_CHANGE_BATCHES_EXCEPTION] wallet_id={wallet_id}, "
            f"error={str(e)}",
            exc_info=True
        )
        return {
...
async def create_token_batch(
    ...
    try:
        ...
        source_type_enum = TransactionSource(source_type) if isinstance(source_type, str) else source_type
        ...
        logger.error(f"Database error creating token batch: {str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
...
async def check_existing_token_batch(idempotency_key: str, session: AsyncSession) -> Dict[str, Any]:
    ...
    result = await session.execute(
        select(TokenBatch.payment_id, TokenBatch.status).where(TokenBatch.idempotency_key == idempotency_key)
    )
    existing_batch = result.fetchone()
    return existing_batch


async def get_token_batch_by_payment_id(payment_id: str, session: AsyncSession) -> Optional[TokenBatch]:
    """
    ...
    try:
        ...
        logger.error(f"Error getting token batch by payment_id: {str(e)}", exc_info=True)
        return None
...
async def confirm_token_batch(
    ...
    try:
        ...
        logger.error(
            f"[CONFIRM_BATCH_DB_ERROR] payment_id={payment_id}, error={str(e)}", 
            exc_info=True
        )
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(
            f"[CONFIRM_BATCH_UNEXPECTED_ERROR] payment_id={payment_id}, error={str(e)}", 
            exc_info=True
        )
...
async def fail_token_batch(
    ...
    try:
        ...
        if payment_method or payment_method_details:
            ...
            # (SQLAlchemy doesn't detect in-place mutations to JSONB columns)
            batch_meta_data = dict(batch.meta_data) if batch.meta_data else {}
            transaction_meta_data = dict(transaction.meta_data) if transaction and transaction.meta_data else {}
            
            # Add failed timestamp to distinguish from completed payments
            batch_meta_data['failed_at'] = datetime.now(timezone.utc).isoformat()
            if transaction:
                transaction_meta_data['failed_at'] = datetime.now(timezone.utc).isoformat()
            ...
            if payment_method_details:
                ...
                method_type = payment_method_details.get('type')
                logger.info(
                    f"[FAIL_BATCH_PAYMENT_DETAILS] batch_id={batch.id}, "
                    f"method_type={method_type}"
                )
        ...
        logger.error(
            f"[FAIL_BATCH_DB_ERROR] payment_id={payment_id}, error={str(e)}", 
            exc_info=True
        )
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(
            f"[FAIL_BATCH_UNEXPECTED_ERROR] payment_id={payment_id}, error={str(e)}", 
            exc_info=True
        )
        return {"success": False, "error": f"Unexpected error: {str(e)}"}


async def get_token_batches_fifo(wallet_id: str, session: AsyncSession, include_expired: bool = True, lock_for_update: bool = False) -> List[TokenBatch]:
    """
...
async def reserve_tokens_fifo(
    ...
    try:
        ...
        if token_reserve_remaining > 0:
            await session.rollback()
            return {"success": False, "error": "Failed to reserve tokens, Insufficient token balance"}
        ...
        logger.error(f"Database error reserving tokens: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(f"Unexpected error reserving tokens: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": str(e)}
...
async def confirm_debit(report_id: str, session: AsyncSession) -> Dict[str, Any]:
    ...
    try:
        ...
        for batch_info in affected_batches:
            ...
            if not batch:
                logger.warning(f"Batch {batch_info['token_batch_id']} not found during confirm_debit for report {report_id}")
                continue
        ...
        logger.error(f"Database error confirming debit: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {str(e)}"}
...
async def release_reservation(
    ...
    try:
        ...
        for batch_info in affected_batches:
            ...
            if not batch:
                logger.warning(f"Batch {batch_info['token_batch_id']} not found during release for report {report_id}")
                continue
        ...
        logger.error(f"Database error releasing reservation: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {str(e)}"}
...
async def credit_tokens_from_payment(
    ...
    try:
        ...
        transaction = TokenTransaction(
            ...
            status=TransactionStatus.COMPLETED,
            description=description or f"{source_type.replace('_', ' ').title()} - {tokens:,} tokens",
            meta_data={"subscription_tier": subscription_tier, "payment_id": payment_id, "amount": amount, "currency": currency} if subscription_tier else {"payment_id": payment_id, "amount": amount, "currency": currency}
        )
        session.add(transaction)
        await session.commit()
        ...
        logger.error(f"Database error crediting tokens: {str(e)}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {str(e)}"}
...
async def get_transaction_history(
    ...
    try:
        ...
        base_filters = [
            ...
            TokenTransaction.status != TransactionStatus.INVALIDATED,  # Exclude invalidated transactions from plan changes
            ...
            or_(
                ...
                TokenBatch.start_at <= datetime.now(timezone.utc),
            ),
        ]
        ...
        for transaction, batch, interval in rows:
            ...
            if interval and batch and batch.source_type == TransactionSource.SUBSCRIPTION:
                ...
                display_amount = float(interval.amount) if interval.amount else (float(batch.amount) if batch and batch.amount else None)
                display_currency = interval.currency if interval.currency else (batch.currency if batch else None)
            else:
        ...
        logger.error(
            f"[GET_TRANSACTION_HISTORY_DB_ERROR] user_id={user_id}, error={str(e)}", 
            exc_info=True
        )
        logger.error(f"Database error getting transactions: {str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
...
def verify_webhook_api_key(provided_key: Optional[str]) -> bool:
    """
