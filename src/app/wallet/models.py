...
class Wallet(Base):
    ...
    available_balance = Column(
        Integer, nullable=False, default=0, comment="Tokens available for use"
    )
    reserved_balance = Column(
        Integer, nullable=False, default=0, comment="Tokens locked for pending operations"
    )
    ...
    token_batches = relationship(
        "TokenBatch", back_populates="wallet", cascade="all, delete-orphan"
    )
    ...
    def __repr__(self):
        return (
            f"<Wallet(id='{self.id}', user_id='{self.user_id}', balance={self.available_balance})>"
        )
...
class TokenBatch(Base):
    ...
    remaining_tokens = Column(
        Integer, nullable=False, comment="Available tokens (source of truth for balance)"
    )
    reserved_tokens = Column(
        Integer, nullable=False, default=0, comment="Tokens locked for pending operations"
    )
    ...
    payment_id = Column(
        String(50), nullable=True, index=True, comment="Reference to payment_id in payment service"
    )
...
class TokenTransaction(Base):
    ...
    balance_after = Column(
        Integer, nullable=False, comment="Available balance snapshot after transaction"
    )
    reserved_after = Column(
        Integer, nullable=False, comment="Reserved balance snapshot after transaction"
    )

    # References
    report_id = Column(
        CHAR(36), nullable=True, index=True, comment="Reference to report (if applicable)"
    )
    token_batch_id = Column(
        CHAR(36),
        ForeignKey("token_batches.id", ondelete="SET NULL"),
        nullable=True,
