...
from app.core.constants import CASPR_PAYMENT_API_KEY, CASPR_PAYMENT_BASE_URL
...
class PaymentService:
    ...
    async def _make_request(
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_error = f"Timeout calling payment service: {e!s}"
                ...
                last_error = f"Network error calling payment service: {e!s}"
                logger.warning(f"Attempt {attempt + 1}/{max_retries} - {last_error}")
                if attempt < max_retries - 1:
                ...
                logger.error(f"Error calling payment service: {e!s}", exc_info=True)
                return {"success": False, "error": f"Payment service error: {e!s}"}
    ...
    async def create_topup_order(
        ...
        payload = {
            ...
            "user_data": {"user_id": user_id, "email": email, "tokens_to_credit": tokens},
        }
    ...
    async def get_payment_order(
        cls, payment_id: str | None = None, idempotency_key: str | None = None
    ) -> dict[str, Any]:
        """
    ...
    async def create_subscription(
        ...
        """
        logger.info(
            f"Creating subscription: plan={plan_tier}, duration={plan_duration}, country={country}"
        )
    ...
    async def cancel_subscription(
        cls, subscription_id: str, cancel_at_cycle_end: bool = True, reason: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        payload = {"subscription_id": subscription_id, "cancel_at_cycle_end": cancel_at_cycle_end}
