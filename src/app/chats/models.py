...
class Message(Base):
    ...
    message_citations = Column(
        JSONB,
        nullable=True,
        comment="Citations keyed by LangChain AIMessage id: {ai_msg_id: [url, ...]}",
    )
    created_at = Column(
        DateTime(timezone=True), nullable=False, comment="Timestamp when message was created"
    )
    updated_at = Column(
        DateTime(timezone=True), nullable=False, comment="Timestamp when message was last updated"
    )
    is_deleted = Column(
        Boolean,
        default=False,
    ...
    deleted_at = Column(
        DateTime(timezone=True), nullable=True, comment="Timestamp when the chat was deleted"
    )
    ...
    def __repr__(self):
        return (
            f"<Message(id='{self.id}', user_id='{self.user_id}', created_at='{self.created_at}')>"
        )
...
Index(
    ...
    AskCasprChat.report_id,
    AskCasprChat.section_id,
    AskCasprChat.subsection_id,
    AskCasprChat.created_at,
)
