...
#: Redis key suffix holding the thread of a graph paused for report config.
_REPORT_CONFIG_PENDING_SUFFIX = "report_config_pending"
#: How long a pause stays confirmable. A Confirm arriving later is refused
#: rather than starting a retrieval nobody configured.
_REPORT_CONFIG_PAUSE_TTL = timedelta(hours=24)
...
def report_config_pending_key(chat_id: str) -> str:
    """Where a paused turn's thread id is parked until it is confirmed."""
    return f"chat:{chat_id}:{_REPORT_CONFIG_PENDING_SUFFIX}"


async def _record_report_config_pause(
    chat_id: str, session_id: str, message: str, output: dict
) -> dict:
    """Park the paused thread against the chat and tell the frontend (R2).

    Recording comes first: the pause event is useless to the frontend if the
    Confirm it provokes cannot find the thread it names.

    The user's message rides along because the paused turn saves no history —
    its last message is an unanswered tool call. The resume is what writes the
    turn to the database, and it needs the message that started it.
    """
    thread_id = output.get("thread_id", "")
    default_tier = output.get("default_tier", "study")

    await redis_instance.redis_client.set(
        report_config_pending_key(chat_id),
        json.dumps(
            {
                "thread_id": thread_id,
        ...
        ex=int(_REPORT_CONFIG_PAUSE_TTL.total_seconds()),
    )

    payload = {
        "type": "report_config_required",
        "thread_id": thread_id,
        "default_tier": default_tier,
        "tiers": output.get("tiers", []),
        "styles": output.get("styles", []),
        "layouts": output.get("layouts", []),
    }
    await redis_instance.redis_client.xadd(
        f"session:{session_id}", {"event": "message", "data": json.dumps(payload)}
    )
    logger.info(
        f"Paused for report configuration: chat_id={chat_id} thread_id={thread_id}"
    )
    return payload
...
async def chat_producer(
    ...
    resume_submission: dict | None = None,
):
    """Chat endpoint with streaming response.

    When ``resume_thread_id`` is given this is the second half of a turn that
    paused for report configuration: the graph is re-entered on the paused
    thread with the confirmed submission instead of a new user message, and
    everything downstream — the event loop, report creation, persistence — is
    the same code an ordinary turn runs.
    """
    ...
    if not is_resume:
        # The Confirm endpoint already claimed the turn lock for a resume, and
        # it set the key to the message the paused turn started from.
        await redis_instance.redis_client.set(
    ...
    try:
            ...
            lambda: Casper(
                {
                    ...
                    # A resume must land on the paused thread, so the turn's
                    # thread id is handed back rather than minted afresh.
        ...
        # Set when the graph parks at the report-config interrupt. The turn is
        # not finished at that point, so none of the end-of-turn persistence
        # below may run — the resume request finishes the turn instead.
        paused_for_report_config = False

        if is_resume:
            graph_state = casper.resume_report_config(resume_submission or {})
        else:
        ...
        async for mode, output in graph_state:
            if mode == "messages":
                ...
                if custom_name == "layout_pair":
                    # One event per tier, each tagged, so the popup can render
                    # both and toggle between them without asking for more.
                    await redis_instance.redis_client.xadd(
                        stream_key,
                        {
                            "event": "message",
                            "data": json.dumps(
                                {
                                    "type": "report_layout_option",
                                    "report_tier": output.get("report_tier", ""),
                                    "report_title": output.get("report_title", ""),
                                    "data": output.get("report_layout", []),
                                }
                            ),
                        },
                    )

                elif custom_name == "report_config" and custom_status == "awaiting_configuration":
                    paused_for_report_config = True
                    await _record_report_config_pause(
                        data.chat_id, data.session_id, data.message, output
                    )

                elif custom_name in (
                    "retrieve_latest_info",
                    "query_document",
                ) and custom_status in (
                    "start",
                    "heartbeat",
                    "end",
                ):
        ...
        if paused_for_report_config:
            # The turn is parked mid-flight: its last message is an unanswered
            # tool call. Saving that as history would corrupt the conversation,
            # and the resume request re-enters this function to finish the turn.
            logger.info(
                f"Stream ended at the report-config pause for chat_id: {data.chat_id} — "
                f"awaiting a Confirm submission"
            )
            return
...
@router.post(
    ...
    status_code=200,
    response_class=JSONResponse,
    summary="Confirm the report configuration for a paused chat",
    description="Resume a chat that paused before retrieval, using the confirmed configuration",
    responses={
        200: {"content": {"application/json": {}}},
        401: {"model": ErrorResponse},
...
async def confirm_report_config(
    data: ReportConfigRequest,
    background_tasks: BackgroundTasks,
    user_id: str = Depends(get_current_active_user),
):
    """POST /chat/report-config – the only thing that resumes a paused graph.

    The chat paused before retrieval and told the frontend which thread it
    parked on. This endpoint takes that thread id back with the user's
    configuration and drives the same turn to report generation. An ordinary
    chat message does not resume a pause (KD7); this is the single entry point.
    """
    logger.info(
        f"Received report-config confirmation for user_id: {user_id}, "
    ...
    try:
        ...
        pending_key = report_config_pending_key(data.chat_id)
        pending_raw = await redis_instance.redis_client.get(pending_key)
        if not pending_raw:
            # Either nothing ever paused, the pause was already confirmed, or
            # the record aged out. Any of those must not start a retrieval.
            logger.info(f"No pending report-config pause for chat_id={data.chat_id}")
            return JSONResponse(
                status_code=409,
                content={
                    "success": False,
                    "error": "This chat is not waiting for a report configuration.",
                    "error_code": "NO_PENDING_REPORT_CONFIG",
                },
            )
        ...
        if pending.get("thread_id") != data.thread_id:
            logger.warning(
                f"Report-config thread mismatch for chat_id={data.chat_id}: "
                f"submitted={data.thread_id}, pending={pending.get('thread_id')}"
            )
            return JSONResponse(
                status_code=409,
                content={
                    "success": False,
                    "error": "This configuration is for a report request that is no longer waiting.",
                    "error_code": "STALE_REPORT_CONFIG_THREAD",
                },
            )
        ...
        # The paused turn saved no history, so the message that started it comes
        # back from the pause record rather than from the database.
        paused_message = str(pending.get("message") or "")

        # Claim the turn before consuming the pause, so a second Confirm racing
        # the first is refused rather than resuming the same thread twice.
        was_set = await redis_instance.redis_client.set(
            f"chat:{data.chat_id}:processing_user_message",
            json.dumps({"type": "human", "content": paused_message}),
            ex=int(timedelta(hours=24).total_seconds()),
            nx=True,
        )
        if not was_set:
            logger.info(
                f"Rejecting report-config confirmation for chat_id={data.chat_id} "
                f"(turn already in progress)"
            )
            return JSONResponse(
                status_code=409,
        ...
        # A pause is confirmable exactly once.
        await redis_instance.redis_client.delete(pending_key)

        submission = data.model_dump(exclude={"chat_id", "session_id", "thread_id"})
        resume_request = ChatRequest(
            message=paused_message, chat_id=data.chat_id, session_id=data.session_id
        )
        ...
        logger.info(
            f"Resuming paused chat_id={data.chat_id} on thread_id={data.thread_id} "
            f"with tier='{data.report_tier}'"
        )
        return JSONResponse(
            status_code=200, content={"success": True, "message": "Report generation resumed"}
        )

    except RequestValidationError as e:
        logger.error(f"Validation error in report-config endpoint: {e}")
        return JSONResponse(
            status_code=422,
            content={
                "success": False,
        ...
        logger.error(f"Error in report-config endpoint: {e}")
        return JSONResponse(
            status_code=500,
            content={
                "success": False,
