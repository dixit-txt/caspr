...
def _invoke_anthropic_viz_graph_refine(
    ...
    *,
    ...
    structured_llm = ANTHROPIC_VIZ_LLM.with_structured_output(
        HTMLVisualizationRefine, include_raw=True
    )
...
def _validate_refine_html_for_pdf(
    html_code: str,
    table_data: str,
    user_prompt: str = None,
    chat_id: str | None = None,
    user_id: str | None = None,
) -> HTMLRefineValidationResult:
...
Check if the HTML will render properly in PDF, looks professional, and correctly represents the table data."""
    ...
    try:
        ...
        save_raw_llm_response(
            response,
            OPENAI_VALIDATION_MODEL,
            "Checking a refined chart visualization",
            chat_id,
            user_id=user_id,
        )
        ...
        try:
            ...
            save_raw_llm_response(
                interaction,
                GEMINI_VALIDATION_MODEL,
                "Checking a refined chart visualization (backup)",
                chat_id,
                user_id=user_id,
            )
...
def generate_html_visualization_refine(
    table_data: str,
...
Analyze the above data and refine the existing visualization (or generate a new one if no existing viz provided) following the user's refinement instructions. You MUST provide all three fields: html_code, visualization_types, and reasoning."""
    ...
    for attempt in range(MAX_VALIDATION_RETRIES):
        logger.info(
            f"[GraphRefine] Validating HTML (attempt {attempt + 1}/{MAX_VALIDATION_RETRIES})"
        )
        validation = _validate_refine_html_for_pdf(
        ...
        logger.warning(
            f"[GraphRefine] Validation failed ({validation.severity}): {validation.issues}"
        )

        user_instruction_context = (
            f"\n\nUSER REFINEMENT INSTRUCTION (must be followed): {user_refine_prompt}"
            if user_refine_prompt
            else ""
        )
...
Generate a corrected HTML visualization that addresses all the issues above. Ensure it renders cleanly in PDF."""
        ...
        try:
            regen_result = _invoke_refine_with_fallback(
                fix_user_prompt, chat_id=chat_id, user_id=user_id
            )
