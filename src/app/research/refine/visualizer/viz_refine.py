...
from app.core.constants import S3_REPORTS_BASE_PATH, VISUALIZATION_MODEL
...
from app.research.prompts.prompt_utils import TABLE_TO_VIZ_PROMPT
from app.research.refine.visualizer.html_graph_maker_refine import (
    generate_html_visualization_refine,
)
from app.research.refine.visualizer.html_table_visualizer_refine import (
    generate_info_visualization_refine,
)
...
def generate_refine_table_visualization(
    ...
    if is_quantitative:
        try:
            ...
            image_html = generate_html_visualization_refine(
                table,
                user_refine_prompt=user_prompt,
            ...
            if image_html is not None:
                logger.info(
                    f"HTML graph visualization refine succeeded: {image_html.visualization_types}"
                )
                return image_html.html_code
        except Exception as e:
            logger.error(f"HTML graph visualization refine failed: {e!s}")
    else:
        try:
            # raise Exception("test")
            logger.info("Decision=False: Trying infographic visualization refine")
            info_viz = generate_info_visualization_refine(
                table,
                user_refine_prompt=user_prompt,
                existing_viz=existing_viz,
                chat_id=chat_id,
            ...
            if info_viz is not None:
                logger.info(
                    f"Infographic visualization refine succeeded: {info_viz.visualization_type}"
                )
                return info_viz.html_code
        except Exception as e:
            logger.error(f"Infographic visualization refine failed: {e!s}")

    # Step 3: Fallback to Gemini image generation (table + user_ins only, NOT existing viz)
    REFINE_VIZ_PROMPT = (
        TABLE_TO_VIZ_PROMPT.format(table=table)
        + f" Make it in English only {user_prompt}(ENGLISH ONLY)"
        if user_prompt
        else TABLE_TO_VIZ_PROMPT.format(table=table)
    )
    last_error = None
    try:
        # raise Exception("test")
        logger.info("Using Gemini for visualization (table + user_ins only)")
        for attempt in range(1, max_retries + 1):
            api_key_num = ((attempt - 1) % 6) + 1
            Google_client = genai.Client(api_key=os.getenv(f"GEMINI_API_KEY_{api_key_num}"))
            try:
                logger.info(
                    f"Generating table visualization (attempt {attempt}/{max_retries} using GEMINI_API_KEY_{api_key_num})"
                )
                ...
                logger.error(
                    f"Attempt {attempt}/{max_retries} (GEMINI_API_KEY_{api_key_num}) failed: {e}"
                )
                last_error = e
        ...
        logger.error(f"Error generating visualization with Gemini: {e!s}")
...
def generate_visualization(
    ...
    try:
        image_path = generate_refine_table_visualization(
            table, user_prompt, chat_id, existing_viz=existing_viz, user_id=user_id
        )
...
def refine_viz(input_json: dict, user_name: str, chat_id: str, user_id: str | None = None) -> str:
    """
    ...
    try:
        ...
        Refined_viz_path = generate_visualization(
            table, user_prompt, user_name, chat_id, existing_viz=existing_viz, user_id=user_id
        )
        if not Refined_viz_path:
            logger.error("No Refined visualization is generated")
            return ""
        return Refined_viz_path
    except Exception as e:
        logger.error(f"Error refining viz: {e!s}")
        return ""
