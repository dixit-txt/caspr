...
class DomainConfig:
    ...
    tone: str = "Trusted Senior Analyst (conclusions first, numbers-led, no hedging)"
...
async def generate_section_cards(
        ...
        while True:
            ...
            try:
                ...
                card = replace_brief_citations(
                    raw_card, citation_url_map, all_report_citations, keep_labels=True
                )
    ...
    for idx, section in enumerate(sections):
        ...
        try:
            ...
            # Keep the model's inline [Source Name](url) citations as written and
            # only clean the URLs. This used to call `replace_citations`, which
            # renumbered every link to [N] — `_report.py` dropped it for that
            # reason, but this domain path kept it, so due-diligence and
            # primary-research reports still came out numbered.
            card["content"] = (
