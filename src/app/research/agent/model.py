...
from app.adapters.grep_agent_2 import PdfSession, ask_pdfs
...
class Casper:
    ...
    def _initialize_components(self) -> None:  # Added: New method to initialize components
        ...
        try:
            ...
            if self.web_search:
                logger.info(
                    f"Live information retrieval mode enabled for user: {self.user_name} - chat_id: {self.chat_id}"
                )
...
Make sure to cite your sources appropriately."""
                ...
                try:
                    response = await ASYNC_OPENAI_CLIENT.responses.parse(
                        ...
                        input=[
                            {
                                "role": "system",
                                "content": "You are a helpful assistant that searches the web to provide accurate, up-to-date information with proper citations.",
                            },
    ...
    async def report_or_respond(self, state: MessagesState) -> dict[str, list]:
        ...
        try:
                    ...
                    async with async_session_scope() as session:
                        ...
                        if balance_info.get("success"):
                            ...
                            has_sufficient_balance = (
                                True  # Allow tool binding if check fails, downstream will handle
                            )
            ...
            if not has_sufficient_balance and balance_info:
                ...
                updated_system_msg = SystemMessage(
                    content=original_system_msg.content + insufficient_balance_context
                )
                ...
                try:
                    logger.info(
                        f"[insufficient_balance] Using anthropic llm for user: {self.user_name} - chat_id: {self.chat_id}"
                    )
                    response = await self.anthropic_llm.ainvoke(messages)
                    save_raw_llm_response(
                        response,
                        ANTHROPIC_MODEL_ID,
                        "Handling a low-balance research request",
                        self.chat_id,
                        user_id=self.user_id or self.user_name,
                    )
                    ...
                    logger.error(
                        f"[insufficient_balance] Anthropic failed: {e}, falling back to openai for user: {self.user_name} - chat_id: {self.chat_id}"
                    )
                    try:
                        logger.info(
                            f"[insufficient_balance] Using openai llm for user: {self.user_name} - chat_id: {self.chat_id}"
                        )
                        response = await OPENAI_LLM_LANGCHAIN.ainvoke(messages)
                        save_raw_llm_response(
                            response,
                            OPENAI_CHAT_MODEL_ID,
                            "Handling a low-balance research request (backup)",
                            self.chat_id,
                            user_id=self.user_id or self.user_name,
                        )
                    except Exception as e:
                        logger.error(
                            f"[insufficient_balance] All LLM providers failed for user: {self.user_name} - chat_id: {self.chat_id}. Last error: {e}"
                        )
                ...
                if web_search_count < MAX_WEB_SEARCH_CALLS_PER_TURN:
                    ...
                    logger.info(
                        f"retrieve_latest_info limit reached ({web_search_count}/{MAX_WEB_SEARCH_CALLS_PER_TURN}) for user: {self.user_name} - chat_id: {self.chat_id}"
                    )
            ...
            if report_type == "brief":
                        ...
                        if raw_card is None:
                            ...
                            try:
                                ...
                                card = replace_brief_citations(
                                    raw_card, citation_url_map, all_report_citations
                                )
                ...
                for idx, section in enumerate(descriptive_report_layout):
                    ...
                    try:
                        ...
                        try:
                            section_content_with_citations = await asyncio.to_thread(
                                replace_citations,
                                card["content"],
                                citation_indices,
                                processed_citations,
                            ...
                            if not section_content_with_citations:
                                logger.error(
                                    f"Failed to replace citations in section {idx + 1} for user: {self.user_name} - chat_id: {self.chat_id}"
                                )
                                section_content_with_citations = card["content"]

                            card["content"] = section_content_with_citations
                        except Exception as e:
                            logger.error(f"Error replacing citations in section {idx + 1}: {e!s}")
                            section_content_with_citations = card["content"]
                            card["content"] = section_content_with_citations
                        ...
                        for i, sentence in enumerate(sub_section_content):
                            try:
                                sentence_with_citations = await asyncio.to_thread(
                                    replace_citations,
                                    sentence,
                                    citation_indices,
                                    processed_citations,
                                ...
                                if not sentence_with_citations:
                                    logger.error(
                                        f"Failed to replace citations in subsection {i} for user: {self.user_name} - chat_id: {self.chat_id}"
                                    )
                                    sentence_with_citations = sentence

                                card["sub_sections"][i]["content"] = sentence_with_citations
                            except Exception as e:
                                logger.error(f"Error replacing citations in subsection {i}: {e!s}")
    ...
    def _route_after_gate(self, state: MessagesState) -> str:
        ...
        if any(
            (c.get("name") if isinstance(c, dict) else getattr(c, "name", None)) == "retrieve"
            for c in tool_calls
        ):
            return "tools"
        ...
        try:
            ...
            if not self.user_previous_messages:
                ...
                for idx, msg in enumerate(self.user_previous_messages):
                    ...
                    stop_reason = (
                        _meta.get("stopReason", "")
                        or _meta.get("stop_reason", "")
                        or _meta.get("finish_reason", "")
                    )
                    if msg["type"] == "ai" and stop_reason in ("tool_use", "tool_calls"):
                        if (
                            idx + 1 < len(self.user_previous_messages)
                            and self.user_previous_messages[idx + 1]["type"] == "tool"
                        ):
                            # Advance past all consecutive tool messages (handles parallel tool calls)
                            end = idx + 1
                            while (
                                end < len(self.user_previous_messages)
                                and self.user_previous_messages[end]["type"] == "tool"
                            ):
                                end += 1

                            if (
                                end < len(self.user_previous_messages)
                                and self.user_previous_messages[end]["type"] == "ai"
                            ):
                                continue
                            else:
                                logger.info(
                                    f"tool_use message did not have a corresponding end_turn message, removing it from the chat history for user: {self.user_name} - chat_id: {self.chat_id}"
                                )
                                should_include[idx - 1 : end] = [False] * (end - (idx - 1))

                        else:
                            logger.info(
                                f"tool_use message did not have a corresponding tool type message, removing it from the chat history for user: {self.user_name} - chat_id: {self.chat_id}"
                            )
                        ...
                        if (
                            idx + 1 < len(self.user_previous_messages)
                            and self.user_previous_messages[idx + 1]["type"] != "ai"
                        ):
                            logger.info(
                                f"human message did not have a corresponding ai message, removing it from the chat history for user: {self.user_name} - chat_id: {self.chat_id}"
                            )
                            should_include[idx] = False

                self.user_previous_messages = [
                    msg
                    for msg, include in zip(self.user_previous_messages, should_include)
                    if include
                ]

                # Strip any leading tool messages — orphaned from a previously corrupted history
                while (
                    self.user_previous_messages and self.user_previous_messages[0]["type"] == "tool"
                ):
                    logger.warning(
                        f"Removing orphaned leading tool message from chat history for user: {self.user_name} - chat_id: {self.chat_id}"
                    )
                ...
                if (
                    self.user_previous_messages
                    and self.user_previous_messages[0]["type"] == "system"
                ):
            ...
            return self.graph.astream(
                initial_state, stream_mode=["messages", "values", "updates", "custom"]
            )
        except Exception as e:
            # import traceback
