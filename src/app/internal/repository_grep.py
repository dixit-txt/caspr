...
from app.core.enums import UploadedFileStatus
from app.core.logging import setup_logging
from app.models import ChatFile, UploadedFile, UploadedFileChunk
...
async def store_parsed_file_metadata(
    ...
    try:
        ...
        uploaded_file.parsed_at = datetime.now(UTC)
        await session.flush()
        logger.info(
...
async def get_chat_uploaded_file_ids(
    ...
    try:
        ...
        logger.error(f"[grep_db] DB error fetching chat file ids for chat_id={chat_id}: {e}")
        raise
