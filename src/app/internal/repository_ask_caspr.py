...
from src.db.database import Card, Report, AskCasprChat, RefinementHistory, ReportVersionCard
...
async def validate_section_exists_in_ask_caspr(
    ...
    """
    logger.info(f"[DB_VALIDATE_SECTION] Starting validation | report_id={report_id} | section_id={section_id} | subsection_id={subsection_id}")
    ...
    try:
        ...
        stmt = stmt.limit(1)
        logger.info(f"[DB_VALIDATE_SECTION] Executing query")
        ...
        if entry is None:
            if subsection_id:
                logger.warning(f"[DB_VALIDATE_SECTION] Subsection not found | report_id={report_id} | section_id={section_id} | subsection_id={subsection_id}")
                return {
                    "success": True,
                    "exists": False,
                ...
                logger.warning(f"[DB_VALIDATE_SECTION] Section not found | report_id={report_id} | section_id={section_id}")
                return {
                    "success": True,
                    "exists": False,
        ...
        logger.info(f"[DB_VALIDATE_SECTION] Validation successful | entry_id={entry.id} | version={entry.version}")
        return {
            "success": True,
            "exists": True,
            "entry_id": entry.id
        ...
        logger.error(f"[DB_VALIDATE_SECTION] Database error | report_id={report_id} | section_id={section_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(f"[DB_VALIDATE_SECTION] Unexpected error | report_id={report_id} | section_id={section_id} | error={str(e)}", exc_info=True)
...
async def get_latest_ask_caspr_chat_entry(
    ...
    """
    logger.info(f"[DB_GET_CHAT_ENTRY] Fetching latest chat entry | report_id={report_id} | section_id={section_id} | subsection_id={subsection_id}")
    ...
    try:
        ...
        logger.info(f"[DB_GET_CHAT_ENTRY] Executing query")
        
        result = await session.execute(stmt)
        entry = result.scalar_one_or_none()
        
        if entry is None:
            logger.warning(f"[DB_GET_CHAT_ENTRY] No chat entry found | report_id={report_id} | section_id={section_id} | subsection_id={subsection_id}")
            return {
                "success": True,
                "entry": None,
                "chat": None,
        ...
        logger.info(f"[DB_GET_CHAT_ENTRY] Chat entry found | entry_id={entry.id} | version={entry.version} | chat_length={chat_length} | created_at={entry.created_at}")
        return {
            "success": True,
            "entry_id": entry.id,
            "chat": entry.chat,
        ...
        logger.error(f"[DB_GET_CHAT_ENTRY] Database error | report_id={report_id} | section_id={section_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(f"[DB_GET_CHAT_ENTRY] Unexpected error | report_id={report_id} | section_id={section_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {str(e)}"}
...
async def update_ask_caspr_chat_conversation(
    ...
    """
    logger.info(f"[DB_UPDATE_CHAT] Updating chat conversation | entry_id={entry_id} | new_messages_count={len(chat)}")
    ...
    try:
        ...
        logger.info(f"[DB_UPDATE_CHAT] Found entry | entry_id={entry_id} | old_chat_length={old_chat_length} | new_chat_length={len(chat)}")
        
        entry.chat = chat
        flag_modified(entry, "chat")
        await session.commit()
        
        logger.info(f"[DB_UPDATE_CHAT] Chat updated successfully | entry_id={entry_id} | messages_count={len(chat)}")
        return {"success": True}
    
    except SQLAlchemyError as e:
        await session.rollback()
        logger.error(f"[DB_UPDATE_CHAT] Database error | entry_id={entry_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        await session.rollback()
        logger.error(f"[DB_UPDATE_CHAT] Unexpected error | entry_id={entry_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {str(e)}"}
...
async def update_ask_caspr_chat_after_refine(
    ...
    try:
        ...
        logger.error(
            f"[DB_UPDATE_AFTER_REFINE] Database error | entry_id={entry_id} | error={str(e)}",
            exc_info=True,
        )
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(
            f"[DB_UPDATE_AFTER_REFINE] Unexpected error | entry_id={entry_id} | error={str(e)}",
            exc_info=True,
        )
        return {"success": False, "error": f"Unexpected error: {str(e)}"}
...
async def get_card_data_for_ask_caspr(
    ...
    """
    logger.info(f"[DB_GET_CARD_DATA] Fetching card data | report_id={report_id} | section_id={section_id} | subsection_id={subsection_id}")
    ...
    try:
        ...
        logger.info(f"[DB_GET_CARD_DATA] Executing query")
        result = await session.execute(stmt)
        card = result.scalar_one_or_none()
        
        if card is None:
            logger.warning(f"[DB_GET_CARD_DATA] No active card found | section_id={section_id} | report_id={report_id}")
            return {"success": False, "error": "Card not found for the given section"}
        
        logger.info(f"[DB_GET_CARD_DATA] Card found | card_id={card.id} | card_title={card.title} | version={card.version}")
        ...
        if subsection_id:
            ...
            logger.info(f"[DB_GET_CARD_DATA] Looking for subsection | subsection_id={subsection_id}")
            ...
            if subsection_data is None:
                logger.warning(f"[DB_GET_CARD_DATA] Subsection not found | subsection_id={subsection_id} | section_id={section_id}")
            ...
            logger.info(f"[DB_GET_CARD_DATA] Subsection data extracted | name={section_or_subsection[:80]}... | content_length={len(content)}")
        else:
            ...
            if isinstance(card.content, dict):
                ...
                logger.info(f"[DB_GET_CARD_DATA] Section data from content dict | name={section_or_subsection[:80]}... | content_length={len(content)}")
            else:
                section_or_subsection = card.title or ''
                content = card.content or ''
                logger.info(f"[DB_GET_CARD_DATA] Section data from card fields | name={section_or_subsection[:80]}... | content_length={len(content)}")
        
        logger.info(f"[DB_GET_CARD_DATA] Card data fetch successful | section={section_or_subsection[:80]}... | citations_count={len(citations)}")
        return {
            "success": True,
        ...
        logger.error(f"[DB_GET_CARD_DATA] Database error | report_id={report_id} | section_id={section_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(f"[DB_GET_CARD_DATA] Unexpected error | report_id={report_id} | section_id={section_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {str(e)}"}


async def get_report_data_for_ask_caspr(
    report_id: str,
    session: AsyncSession
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"[DB_GET_REPORT_DATA] Executing query")
        result = await session.execute(stmt)
        ...
        logger.info(f"[DB_GET_REPORT_DATA] Report found | report_id={report_id} | title={report.title} | file_id={report.file_id} | chat_id={report.chat_id} | citations_count={citations_count} | has_markdown={bool(report.initial_markdown)}")
        ...
        logger.error(f"[DB_GET_REPORT_DATA] Database error | report_id={report_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(f"[DB_GET_REPORT_DATA] Unexpected error | report_id={report_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {str(e)}"}
...
async def get_refinement_history_for_ask_caspr(
    ...
    try:
        ...
        logger.info(f"[DB_GET_REFINEMENT] Executing query")
        result = await session.execute(stmt)
        ...
        logger.info(f"[DB_GET_REFINEMENT] Refinement history found | report_id={report_id} | refinement_count={refinement_count}")
        return record.refine_history
    
    except Exception as e:
        logger.error(f"[DB_GET_REFINEMENT] Error fetching refinement history | report_id={report_id} | error={str(e)}", exc_info=True)
        return []
...
async def get_ask_caspr_chat_for_version(
    ...
    """
    logger.info(f"[DB_GET_CHAT_VERSION] Fetching chat for version | report_id={report_id} | section_id={section_id} | subsection_id={subsection_id} | report_version_id={report_version_id}")

    try:
        # ── Mode 1: Latest (no report_version_id) ──────────────────────────
        if not report_version_id:
            logger.info(f"[DB_GET_CHAT_VERSION] Mode: LATEST (no version specified)")
            stmt = select(AskCasprChat).where(
                AskCasprChat.report_id == report_id,
                AskCasprChat.section_id == section_id
            )
            ...
            stmt = stmt.order_by(AskCasprChat.created_at.desc()).limit(1)
            logger.info(f"[DB_GET_CHAT_VERSION] Executing latest chat query")
            result = await session.execute(stmt)
            entry = result.scalar_one_or_none()

            if entry is None:
                logger.info(f"[DB_GET_CHAT_VERSION] No chat found (latest mode)")
                return {
                    "success": True,
                    "entry_id": None,
                    "chat": None,
            ...
            chat_length = len(entry.chat) if entry.chat else 0
            logger.info(f"[DB_GET_CHAT_VERSION] Latest chat found | entry_id={entry.id} | version={entry.version} | card_version={entry.card_version} | chat_length={chat_length}")
            return {
                "success": True,
                "entry_id": entry.id,
                "chat": entry.chat,
        ...
        logger.info(f"[DB_GET_CHAT_VERSION] Mode: HISTORICAL | report_version_id={report_version_id}")
        ...
        card_stmt = (
            ...
            .where(
                ReportVersionCard.report_version_id == report_version_id,
                Card.card_id == section_id
            )
        )
        logger.info(f"[DB_GET_CHAT_VERSION] Finding target card version | report_version_id={report_version_id} | section_id={section_id}")
        ...
        if target_card_version is None:
            logger.warning(f"[DB_GET_CHAT_VERSION] No card found for section in version | section_id={section_id} | report_version_id={report_version_id}")
            return {
                "success": True,
                "entry_id": None,
                "chat": None,
        ...
        logger.info(f"[DB_GET_CHAT_VERSION] Target card version found | section_id={section_id} | target_card_version={target_card_version}")
        ...
        chat_stmt = chat_stmt.order_by(
            AskCasprChat.card_version.desc(),
            AskCasprChat.created_at.desc()
        ).limit(1)

        logger.info(f"[DB_GET_CHAT_VERSION] Executing historical chat query | target_card_version={target_card_version}")
        chat_result = await session.execute(chat_stmt)
        entry = chat_result.scalar_one_or_none()

        if entry is None:
            logger.info(f"[DB_GET_CHAT_VERSION] No chat found for historical version | target_card_version={target_card_version}")
            return {
                "success": True,
                "entry_id": None,
                "chat": None,
        ...
        chat_length = len(entry.chat) if entry.chat else 0
        logger.info(f"[DB_GET_CHAT_VERSION] Historical chat found | entry_id={entry.id} | version={entry.version} | card_version={entry.card_version} | target_card_version={target_card_version} | chat_length={chat_length}")
        return {
            "success": True,
            "entry_id": entry.id,
            "chat": entry.chat,
        ...
        logger.error(f"[DB_GET_CHAT_VERSION] Database error | report_id={report_id} | section_id={section_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Database error: {str(e)}"}
    except Exception as e:
        logger.error(f"[DB_GET_CHAT_VERSION] Unexpected error | report_id={report_id} | section_id={section_id} | error={str(e)}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {str(e)}"}
