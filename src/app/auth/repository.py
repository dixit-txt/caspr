...
from app.referrals.repository_codes import generate_unique_referral_code
...
async def get_user_details(user_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Database error retrieving user details: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}
    except Exception as e:
        logger.error(f"Unexpected error retrieving user details: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}
...
async def check_user_by_id(user_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "exists": False, "error": f"User with ID {user_id} not found"}

    except SQLAlchemyError as e:
        logger.error(f"Database error checking user by ID: {e!s}", exc_info=True)
        return {"success": False, "exists": False, "error": f"Database error: {e!s}"}

    except Exception as e:
        logger.error(f"Unexpected error checking user by ID: {e!s}", exc_info=True)
        return {"success": False, "exists": False, "error": f"Unexpected error: {e!s}"}
...
async def verify_user_credentials(
    ...
    try:
        ...
        if not user:
            ...
            return {"success": True, "valid": False, "error": f"User with email {email} not found"}
        ...
        logger.error(f"Database error verifying user credentials: {e!s}", exc_info=True)
        return {"success": False, "valid": False, "error": f"Database error: {e!s}"}

    except Exception as e:
        logger.error(f"Unexpected error verifying user credentials: {e!s}", exc_info=True)
        return {"success": False, "valid": False, "error": f"Unexpected error: {e!s}"}
...
async def create_user(user_data: dict[str, Any], session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"User created successfully with ID: {new_user.id}, referral_code: {referral_code}"
        )
        ...
        logger.error(f"Database error creating user: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}


async def check_user_exists_by_email_or_phone(
    email: str = None, phone: str = None, session: AsyncSession = None
) -> dict[str, Any]:
    """
    ...
    if not email or not phone:
        ...
        return {"success": False, "error": "Provide both email and phone"}
    ...
    try:
        ...
        logger.error(f"Database error checking user existence: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}

    except Exception as e:
        logger.error(f"Unexpected error checking user existence: {e!s}", exc_info=True)
...
async def update_user_password(
    user_id: str, password_hash: str, session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    if not password_hash:
        ...
        return {"success": False, "error": "Invalid password hash: empty value provided"}
    ...
    try:
        ...
        logger.error(f"Database error updating user password: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}

    except Exception as e:
        logger.error(f"Unexpected error updating user password: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def update_user_verification_status(
    ...
    if not email:
        ...
        return {"success": False, "error": "Invalid email: empty value provided"}
    ...
    try:
        ...
        logger.error(f"Database error updating user verification status: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}


async def update_user_tc_verified(
    user_id: str, t_c_verified: bool, session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if result.rowcount == 0:
            ...
            return {"success": False, "error": "User not found or couldn't be updated"}

        await session.commit()
        logger.info(f"T&C verification status updated successfully for user_id: {user_id}")

        return {"success": True, "t_c_verified": t_c_verified}

    except SQLAlchemyError as e:
        logger.error(f"Database error updating T&C verification status: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error updating T&C verification status: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Unexpected error: {e!s}"}

    except Exception as e:
        logger.error(f"Unexpected error updating user verification status: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def get_user_walkover_status(user_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Database error fetching walkover_completed: {e!s}", exc_info=True)
        return {
            "success": False,
            "exists": False,
        ...
        logger.error(f"Unexpected error fetching walkover_completed: {e!s}", exc_info=True)
        return {
            "success": False,
            "exists": False,
...
async def mark_walkover_completed(
    ...
    try:
        stmt = update(User).where(User.id == user_id).values(walkover_completed=True)
        result = await session.execute(stmt)
        ...
        logger.error(f"Database error updating walkover_completed: {e!s}", exc_info=True)
        await session.rollback()
        return {
            "success": False,
            "error": f"Database error: {e!s}",
        ...
        logger.error(f"Unexpected error updating walkover_completed: {e!s}", exc_info=True)
        await session.rollback()
        return {
            "success": False,
            "error": f"Unexpected error: {e!s}",
