...
from app.reports.constants import DEFAULT_DOMAIN_INTERNAL, DOMAIN_DISPLAY_NAMES
from app.reports.repository import _to_external_domain, _to_internal_domain
...
async def get_dashboard_stats(user_id: str, session: AsyncSession) -> dict:
    ...
    try:
        ...
        logger.info(
            f"Found {analysis_completed_count} analysis-completed reports for user {user_id}"
        )
        ...
        logger.error(f"Error getting dashboard stats: {e!s}", exc_info=True)
        return {"success": False, "error": f"Error getting dashboard stats: {e!s}"}
        ...
        "categories": [
            {
                ...
                "display_name": DOMAIN_DISPLAY_NAMES.get(slug, slug.replace("_", " ").title()),
            }
...
async def search_chats_and_reports(
    ...
    try:
        ...
        # Paginated rows
        rows_stmt = (
            base_query.order_by(Message.updated_at.desc().nullslast()).limit(limit).offset(offset)
        )
        ...
        items = []
        for r in rows:
            matched_chat = bool(q_lower and r.chat_title and q_lower in r.chat_title.lower())
            matched_report = bool(q_lower and r.report_title and q_lower in r.report_title.lower())
        ...
        logger.info(f"Home-search returning {len(items)} of {total} hits for user_id={user_id}")
        return {
            "success": True,
            "total": total,
            "limit": limit,
        ...
        logger.error(f"DB error in search_chats_and_reports: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error in search_chats_and_reports: {e!s}", exc_info=True)
