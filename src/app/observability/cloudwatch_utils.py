...
async def insert_cloudwatch_logs(data: dict, user_id: str = None):
    try:
        ...
        await CloudwatchInstance.add_log_messages(log_messages=[json.dumps(request_data)])
        ...
        logger.error(
            f"Error inserting cloudwatch logs for user_id: {user_id}, event: {data.get('event')}, error: {e}"
        )
...
async def insert_cloudwatch_logs_for_caspr_page(request_data: dict):
    try:
        logger.info(
            f"Inserting cloudwatch logs for event: {request_data.get('event')} and email: {request_data.get('email')}"
        )
        ...
        logger.info(
            f"Successfully inserted cloudwatch logs for event: {request_data.get('event')} and email: {request_data.get('email')}"
        )
