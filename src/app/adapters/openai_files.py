...
from app.adapters.s3 import download_file_from_s3_to_local
from app.core.constants import SYNC_OPENAI_CLIENT
    ...
    try:
        with open(file_path, "rb") as f:
            ...
            if confidence < 0.7 or encoding is None:
                logger.warning("Low confidence in encoding detection. Trying fallback encodings.")
                return "utf-8"
...
def convert_markdown_to_html(file_path: str) -> str:
    ...
    html_content = markdown.markdown(
        content, extensions=["tables", "fenced_code", "extra", "nl2br", "sane_lists"]
    )
    return html_content
...
def process_different_file_formats(file_path: str) -> str:
    ...
    logger.info(
        f"[PROCESS_FILE] File details | extension={file_extension} | output_pdf={pdf_file_path}"
    )
    ...
    try:
        ...
        if file_extension == ".md":
            logger.info("[PROCESS_FILE] Converting Markdown to PDF")
            ...
            logger.info("[PROCESS_FILE] Converting DOCX to PDF")
            ...
            try:
                ...
                html_content = create_html_document(html_body, "Word Document")
                logger.info(
                    f"[PROCESS_FILE] DOC converted using textract | text_length={len(text)}"
                )
            except ImportError:
            ...
            logger.info("[PROCESS_FILE] Converting TXT to PDF")
            ...
            logger.info("[PROCESS_FILE] Converting JSON to PDF")
            ...
            logger.info("[PROCESS_FILE] Converting CSV to PDF")
            ...
            logger.info("[PROCESS_FILE] Converting XML to PDF")
            ...
            logger.info("[PROCESS_FILE] Converting HTML to PDF")
            content = read_file_with_encoding(file_path)
            ...
            if "<html" in content.lower():
                ...
                logger.info("[PROCESS_FILE] Injected styles into existing HTML")
            else:
                # Wrap in HTML document
                html_content = create_html_document(content, "HTML Document")
                logger.info("[PROCESS_FILE] Wrapped HTML in document structure")

        elif file_extension == ".xls" or file_extension == ".xlsx":
            logger.info("[PROCESS_FILE] Converting Excel to PDF")
        ...
        if html_content:
            logger.info(
                f"[PROCESS_FILE] Generating PDF | output={pdf_file_path} | html_length={len(html_content)}"
            )
            weasyprint.HTML(string=html_content).write_pdf(pdf_file_path)
            pdf_size = os.path.getsize(pdf_file_path)
            logger.info(
                f"[PROCESS_FILE] PDF generated successfully | path={pdf_file_path} | size={pdf_size} bytes"
            )
        ...
        logger.error(
            f"[PROCESS_FILE] Conversion failed | extension={file_extension} | file_path={file_path} | error={e!s}",
            exc_info=True,
        )
...
def upload_file_to_openai(file_path: str) -> str:
    ...
    logger.info(
        f"[UPLOAD_FILE_OPENAI] File details | size={file_size} bytes | exists={os.path.exists(file_path)}"
    )

    for attempt in range(max_retries):
        try:
            logger.info(f"[UPLOAD_FILE_OPENAI] Upload attempt {attempt + 1}/{max_retries}")
            file = SYNC_OPENAI_CLIENT.files.create(file=open(file_path, "rb"), purpose="assistants")
            logger.info(
                f"[UPLOAD_FILE_OPENAI] Upload successful | file_id={file.id} | filename={file.filename} | attempt={attempt + 1}"
            )
            return file
        except Exception as e:
            logger.error(f"[UPLOAD_FILE_OPENAI] Upload attempt {attempt + 1} failed | error={e!s}")
            if attempt == max_retries - 1:
                logger.error(
                    f"[UPLOAD_FILE_OPENAI] All upload attempts failed | max_retries={max_retries}"
                )
                raise Exception(f"Error uploading file after {max_retries} attempts: {e!s}")
...
def check_if_user_vector_store_exists(vector_store_name: str) -> dict:
    ...
    try:
        ...
        logger.error(f"Error while checking for vector store existence: {e!s}")
        return {
            "vector_store_id": None,
            "vector_store_name": vector_store_name,
            "vector_store_exists": False,
...
def create_user_vector_store(vector_store_name: str):
    ...
    try:
        ...
        if result["vector_store_exists"]:
            ...
            vector_store = SYNC_OPENAI_CLIENT.vector_stores.create(name=vector_store_name)
        ...
        logger.error(f"Error while creating vector store: {e!s}")
        return {
            "vector_store_id": None,
            "vector_store_name": vector_store_name,
            "vector_store_exists": False,
...
def attach_file_to_vector_store(
    ...
    try:
        ...
        logger.error(f"Error while attaching file to vector store: {e!s}")
        return False
...
def create_file_session_for_user_uploaded_files(
    ...
    file_paths = [
        download_file_from_s3_to_local(
            path, os.path.basename(path.replace("s3://", "").split("/", 1)[1])
        )
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_path = {
                executor.submit(process_different_file_formats, path): path for path in file_paths
            }
...
def create_file_session_openai_for_ask_caspr(
    markdown_text: str | None = None, file_s3_path: str | None = None
) -> str:
    """
    ...
    """
    logger.info(
        f"[CREATE_FILE_OPENAI] Starting file session creation | has_markdown={markdown_text is not None} | has_s3_path={file_s3_path is not None}"
    )
    max_retries = 3
    retry_delay = 2
    temp_file_path = None
    ...
    try:
        if markdown_text:
            logger.info(
                f"[CREATE_FILE_OPENAI] Creating temp file from markdown | text_length={len(markdown_text)}"
            )
            ...
            logger.info(
                f"[CREATE_FILE_OPENAI] File downloaded from S3 | temp_path={temp_file_path} | extension={file_extension}"
            )

        else:
            logger.error("[CREATE_FILE_OPENAI] Neither markdown_text nor file_s3_path provided")
        ...
        for attempt in range(max_retries):
            try:
                logger.info(
                    f"[CREATE_FILE_OPENAI] Upload attempt {attempt + 1}/{max_retries} | file={processed_file_path}"
                )
                file = upload_file_to_openai(processed_file_path)
                logger.info(
                    f"[CREATE_FILE_OPENAI] File uploaded successfully | file_id={file.id} | attempt={attempt + 1}"
                )
                return str(file.id)
            except Exception as e:
                logger.error(
                    f"[CREATE_FILE_OPENAI] Upload attempt {attempt + 1} failed | error={e!s}"
                )
                if attempt == max_retries - 1:
                    logger.error(
                        f"[CREATE_FILE_OPENAI] All upload attempts failed | max_retries={max_retries}"
                    )
                    raise Exception(f"Error uploading file: {e!s}")
        ...
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                ...
                logger.warning(
                    f"[CREATE_FILE_OPENAI] Failed to remove temp file | path={temp_file_path} | error={e!s}"
                )

        if (
            processed_file_path
            and os.path.exists(processed_file_path)
            and processed_file_path != temp_file_path
        ):
            try:
                os.remove(processed_file_path)
                logger.info(
                    f"[CREATE_FILE_OPENAI] Cleaned up processed file | path={processed_file_path}"
                )
            except Exception as e:
                logger.warning(
                    f"[CREATE_FILE_OPENAI] Failed to remove processed file | path={processed_file_path} | error={e!s}"
                )
    ...
    try:
        for attempt in range(max_retries):
            try:
                ...
                logger.info(
                    f"Attempt {attempt + 1} failed: {e!s}. Retrying in {retry_delay} seconds..."
                )
                time.sleep(retry_delay)
                if attempt == max_retries - 1:
                    logger.error(f"Error deleting files after {max_retries} attempts: {e!s}")
                    return False
                return True
    except Exception as e:
        logger.error(f"error in deleting files: {e}")
...
def check_if_file_exists_openai(file_id: str) -> bool:
    ...
    try:
        ...
        logger.info(
            f"[CHECK_FILE_OPENAI] File exists | file_id={file_id} | status={getattr(file_obj, 'status', 'N/A')}"
        )
        return True
    except Exception as e:
        logger.warning(f"[CHECK_FILE_OPENAI] File does not exist | file_id={file_id} | error={e!s}")
        return False
...
async def ensure_report_file_id(
    ...
    if file_id:
        try:
            if await asyncio.to_thread(check_if_file_exists_openai, file_id):
                logger.info(
                    f"[{log_prefix}] file_id verified in OpenAI | report_id={report_id} | file_id={file_id}"
                )
                return file_id
            logger.warning(
            ...
            logger.error(
                ...
                f"report_id={report_id} | file_id={file_id} | error={check_error!s}",
                exc_info=True,
            )
            return file_id
    ...
    try:
        logger.info(
            f"[{log_prefix}] Uploading initial markdown to OpenAI | report_id={report_id} | s3_path={file_s3_path}"
        )
        ...
        logger.info(
            f"[{log_prefix}] Upload complete | report_id={report_id} | file_id={new_file_id}"
        )
    except Exception as upload_error:
        logger.error(
            f"[{log_prefix}] Failed to upload initial markdown to OpenAI | "
            f"report_id={report_id} | s3_path={file_s3_path} | error={upload_error!s}",
            exc_info=True,
        )
        return None
    ...
    try:
        ...
        logger.info(
            f"[{log_prefix}] Persisted file_id on report | report_id={report_id} | file_id={new_file_id}"
        )
        ...
        logger.error(
            ...
            f"file_id={new_file_id} | error={db_error!s}",
            exc_info=True,
        )
    ...
    try:
        for attempt in range(max_retries):
            try:
                ...
                time.sleep(retry_delay)
                if attempt == max_retries - 1:
                    logger.error(f"Error deleting file after {max_retries} attempts: {e!s}")
                    return False
    except Exception as e:
        logger.error(f"error in deleting file: {e}")
        return False
...
def check_if_vector_store_id_exists_openai(vector_store_id: str) -> bool:
    ...
    try:
        logger.info(
            f"Checking if vector_store_id {vector_store_id} exists on openai: {vector_store_id}"
        )
