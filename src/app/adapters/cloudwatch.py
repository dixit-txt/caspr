...
class CloudwatchControls:
    ...
    async def __is_log_stream_exists(self, log_stream: str, log_group_name: str):
        log_stream_data = await self.cloudwatch_client.describe_log_streams(
            logGroupName=log_group_name, logStreamNamePrefix=log_stream
        )
    ...
    async def __add_log_event_list(
        ...
        await self.cloudwatch_client.put_log_events(
            logGroupName=log_group_name, logStreamName=log_stream, logEvents=log_event_list
        )

    async def __create_log_stream(
        self, log_stream: str = LOG_STREAM, log_group_name: str = LOG_GROUP_NAME
    ):
        exists = await self.__is_log_stream_exists(
            log_stream=log_stream, log_group_name=log_group_name
        )
        if exists:
            return
    ...
    async def get_log_messages(
        self, log_stream: str, log_group_name: str = SUGGESTION_LOG_GROUP_NAME
    ) -> list[dict]:
        try:
