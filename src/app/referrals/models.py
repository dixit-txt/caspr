...
class Referral(Base):
    ...
    referrer_id = Column(
        ...
        nullable=False,
        comment="User who gave the referral code",
    )
    referee_id = Column(
        CHAR(36),
        ForeignKey("users.id", ondelete="CASCADE"),
        nullable=False,
        comment="User who used the referral code",
    )
    referral_code_used = Column(
        String(12), nullable=False, index=True, comment="The referral code that was used"
    )
    tokens_credited_to_referrer = Column(
        Integer, nullable=False, default=25000, comment="Bonus tokens credited to referrer"
    )
    tokens_credited_to_referee = Column(
        Integer, nullable=False, default=25000, comment="Bonus tokens credited to referee"
    )
    status = Column(
        String(20),
        nullable=False,
