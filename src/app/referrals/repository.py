...
from src.db.database import User, Referral, TokenBatch, Wallet
from src.db.enums import TransactionSource, TransactionStatus
from src.config.log_helper import setup_logging
from src.config.constants import TOKEN_EXPIRY_DAYS
...
async def get_user_by_referral_code(referral_code: str, session: AsyncSession) -> Optional[User]:
    """
    ...
    try:
        ...
        logger.error(f"[REFERRAL_CODE_LOOKUP_ERROR] code={referral_code}, error={str(e)}", exc_info=True)
        return None
...
async def check_user_already_referred(user_id: str, session: AsyncSession) -> bool:
    ...
    try:
        result = await session.execute(
            select(Referral).where(Referral.referee_id == user_id)
        )
        ...
        logger.error(f"[CHECK_REFERRED_ERROR] user_id={user_id}, error={str(e)}", exc_info=True)
...
async def create_referral_record(
    ...
    try:
        ...
        logger.error(
            ...
            f"referee_id={referee_id}, error={str(e)}",
            exc_info=True
        )
        return {
            "success": False,
            "error": f"Database error creating referral record: {str(e)}"
        }
...
async def get_referral_stats(user_id: str, session: AsyncSession) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"[GET_REFERRAL_STATS_ERROR] user_id={user_id}, error={str(e)}",
            exc_info=True
        )
        return {
            "success": False,
...
async def credit_referral_bonus(
    ...
    try:
        logger.info(
            f"[CREDIT_REFERRAL_BONUS_START] user_id={user_id}, "
            f"tokens={tokens}, reason={reason}"
        )
        ...
        logger.error(
            f"[CREDIT_REFERRAL_BONUS_ERROR] user_id={user_id}, "
            f"tokens={tokens}, error={str(e)}",
            exc_info=True
        )
        return {
            "success": False,
            "error": f"Database error crediting tokens: {str(e)}"
        }


async def get_user_referral_code(user_id: str, session: AsyncSession) -> Optional[str]:
    """
    Get user's referral code.
    ...
    try:
        ...
        logger.error(f"[GET_USER_REFERRAL_CODE_ERROR] user_id={user_id}, error={str(e)}", exc_info=True)
        return None
