...
class Subscription(Base):
    ...
    subscription_id = Column(
        String(100),
        nullable=True,
        comment="Reference to external subscription id (Razorpay subscription ID)",
    )
    ...
    intervals = relationship(
        ...
        order_by="desc(SubscriptionInterval.start_date)",
    )
...
class SubscriptionInterval(Base):
    ...
    start_date = Column(
        DateTime(timezone=True), nullable=False, comment="Start of billing interval"
    )
    end_date = Column(DateTime(timezone=True), nullable=False, comment="End of billing interval")

    # Payment tracking
    payment_id = Column(
        String(50), nullable=False, index=True, comment="Reference to payment_id in payment service"
    )
    ...
    # Token tracking
    tokens_credited = Column(
        Integer, nullable=False, default=0, comment="Tokens credited for this interval"
    )
...
Index(
    ...
    SubscriptionInterval.subscription_id,
    SubscriptionInterval.status,
    SubscriptionInterval.end_date,
)
