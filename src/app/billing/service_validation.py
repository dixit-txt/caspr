...
from src.db.database import Subscription, SubscriptionInterval, TokenBatch, Wallet
from src.db.enums import SubscriptionTier, SubscriptionStatus, TransactionSource, TransactionStatus
...
def is_plan_transition_allowed(
    ...
    if old_tier == new_tier and old_duration == new_duration:
        return False, f"You are already subscribed to {new_tier.upper()} {new_duration.upper()}. No changes needed."
...
async def calculate_fair_token_usage(
    ...
    try:
        ...
        logger.error(
            f"[FAIR_TOKEN_USAGE_ERROR] subscription_id={subscription_id}, error={str(e)}",
            exc_info=True
        )
