...
from app.cards.repository import (
    _normalize_subsections_for_response,
    _normalize_tables_for_response,
    insert_card,
)
...
async def get_chat_reports(chat_id: int, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        response = {"success": True, "reports": report_list}

        logger.info(f"Successfully retrieved {len(report_list)} reports for chat_id: {chat_id}")
        return response

    except SQLAlchemyError as e:
        logger.error(f"Database error retrieving report files: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}

    except Exception as e:
        logger.error(f"Unexpected error retrieving report files: {e!s}", exc_info=True)
...
async def get_file_s3_path(id: int, file_type: str, session: AsyncSession) -> dict[str, Any]:
    ...
    if not id:
        ...
        return {"success": False, "error": "Invalid file ID: empty value provided"}

    if not file_type:
        logger.error("Invalid file type: empty value provided")
        return {"success": False, "error": "Invalid file type: empty value provided"}
    ...
    try:
        ...
        if not s3_paths:
            ...
            return {"success": False, "error": f"Report with ID {id} not found"}

        s3_path = s3_paths.get(file_type)
        logger.info(f"S3 path for file with ID {id} and file_type {file_type}: {s3_path}")
        return {"success": True, "s3_uri": s3_path}

    except SQLAlchemyError as e:
        logger.error(f"Database error retrieving S3 path: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}

    except Exception as e:
        logger.error(f"Unexpected error retrieving S3 path: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}


async def create_report(
    report_id: str, chat_id: str, created_at: datetime, s3_uri: dict, session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        refinement_history = RefinementHistory(
            id=str(uuid7()), report_id=report_id, refine_history=None
        )
        ...
        logger.info(
            "Successfully created report with ID: %s, initial version, and refinement history",
            report_id,
        )
        return {
            "success": True,
...
async def update_report(
    report_id: str, update_data: dict[str, Any], session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        active_version_stmt = select(ReportVersion).where(
            ReportVersion.report_id == report_id, ReportVersion.is_active == True
        )
        ...
        if not active_version:
            ...
            return {"success": False, "error": f"No active version found for report {report_id}"}
        ...
        if "last_activity_at" in update_data or "status" in locals():
            timestamp = update_data.pop("last_activity_at", datetime.now(UTC))
        ...
        if title_updated and new_title:
            try:
                ...
                if chat:
                    ...
                    logger.info(
                        f"Updated chat title to '{new_title}' for chat_id: {report.chat_id}"
                    )
                else:
                    logger.warning(
                        f"Chat not found with id: {report.chat_id}, could not update chat title"
                    )
            except Exception as e:
        ...
        logger.error(f"Database error updating report: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error updating report: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def update_specific_version_s3_uri(
    ...
    try:
        ...
        if not version:
            ...
            return {
                "success": False,
                "error": f"Version {version_id} not found for report {report_id}",
            }
        ...
        version.last_activity_at = datetime.now(UTC)
        ...
        logger.info(
            f"Updated s3_uri for specific version {version_id} (v{version.version}) - NOT updating Report table"
        )
        return {"success": True, "s3_uri": merged, "version": version.version}

    except SQLAlchemyError as e:
        logger.error(f"Database error updating specific version s3_uri: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error updating specific version s3_uri: {e!s}", exc_info=True)
        await session.rollback()
        return {"success": False, "error": str(e)}


async def update_report_status_by_chat_or_report_id(
    chat_id: str = None, report_id: str = None, status: str = None, session: AsyncSession = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        update_data = {"status": status, "last_activity_at": datetime.now(UTC)}

        return await update_report(
            report_id=target_report_id, update_data=update_data, session=session
        )

    except Exception as e:
        logger.error(f"Error updating report status: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}


async def update_report_status_if_needed(
    report_id: str, new_status: str, session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if current_status == new_status:
            ...
            logger.debug(
                f"Status already {new_status} for report {report_id}, updating last_activity_at only"
            )
            update_data = {"last_activity_at": now}
            result = await update_report(
                report_id=report_id, update_data=update_data, session=session
            )
        ...
        update_data = {"status": new_status, "last_activity_at": now}
        ...
        logger.error(f"Error checking/updating report status: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}
...
async def get_report_details(report_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Database error retrieving report details: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error retrieving report details: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def verify_report_ownership(
    ...
    try:
        ...
        if not report_result or not report_result.get("success", False):
            error_msg = (
                report_result.get("error", "Failed to retrieve report details.")
                if report_result
                else "Failed to retrieve report details."
            )
            logger.error(
                f"Failed to get report details for report_id: {report_id}, error: {error_msg}"
            )
            return {
                "authorized": False,
                "status_code": 500,
        ...
        logger.error(
            f"Database error verifying report ownership for report_id: {report_id}: {e!s}",
            exc_info=True,
        )
        return {
            "authorized": False,
        ...
        logger.error(
            f"Unexpected error verifying report ownership for report_id: {report_id}: {e!s}",
            exc_info=True,
        )
        return {
            "authorized": False,
...
async def link_card_to_report_version(
    ...
    try:
        ...
        logger.info(
            f"Linked card {card_primary_id} to report version {report_version_id} at sequence {sequence}"
        )
        return {"success": True, "mapping_id": mapping.id}

    except Exception as e:
        logger.error(f"Error linking card to report version: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}


async def finalize_report_version(
    session: AsyncSession, report_id: str, version_id: str = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if version.version != 1:
            logger.warning(
                f"Attempted to finalize version {version.version} for report {report_id}. Only version 1 should be finalized via this function."
            )
            return {
                "success": False,
        ...
        existing_mappings_stmt = (
            select(ReportVersionCard)
        ...
        cards_stmt = (
            select(Card)
            .where(Card.report_id == report_id, Card.is_active == True, Card.is_deleted == False)
        ...
        if not cards:
            ...
            return {"success": False, "error": "No active cards found for this report"}
        ...
        logger.info(
            f"Finalized version {version.id} (v{version.version}) with {len(cards)} cards for report {report_id}"
        )
        return {
            "success": True,
        ...
        logger.error(f"Error finalizing report version: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}


async def create_new_report_version(
    session: AsyncSession, report_id: str, cards_data: list, previous_version_id: str = None
) -> dict[str, Any]:
    """
    ...
    try:
            ...
            if card_info.get("is_modified", False):
                ...
                if card_info.get("pk_id"):
                    ...
                    card_result = await insert_card(
                        session=session, report_id=report_id, report_card=card_info
                    )
                    if not card_result.get("success"):
                        raise Exception(
                            f"Failed to insert modified card: {card_result.get('error')}"
                        )
                ...
                if not card_pk_id:
                    logger.error("Missing existing_card_pk_id for unchanged card")
                    continue
        ...
        report.last_activity_at = datetime.now(UTC)
        ...
        await session.rollback()
        logger.error(f"Error creating new report version: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}


async def detect_modified_cards(
    report_id: str, session: AsyncSession, generation_type: str = "report"
) -> dict[str, Any]:
    """
    ...
    """
    logger.info(
        f"Detecting modified cards for report: {report_id}, requested by: {generation_type}"
    )
    ...
    try:
        ...
        if not active_version:
            logger.error(f"No active version found for report {report_id}")
            return {"success": False, "error": "No active version found for this report"}

        # Get all active cards for this report first
        cards_stmt = (
            select(Card)
            .where(Card.report_id == report_id, Card.is_active == True)
            .order_by(Card.sequence)
        )
        ...
        if timestamps:
            ...
            logger.info(
                f"Using most recent generation timestamp from {comparison_source}: {comparison_timestamp}"
            )
        ...
        if not comparison_timestamp:
            ...
            logger.info(
                "No previous generation found for this version - first generation, treating all cards as unchanged"
            )
        ...
        for card in all_cards:
            ...
            if last_modified > comparison_timestamp:
                ...
                logger.info(
                    f"Card {card.card_id} is MODIFIED (updated: {last_modified}, comparison: {comparison_timestamp})"
                )
            else:
                ...
                logger.debug(
                    f"Card {card.card_id} is UNCHANGED (updated: {last_modified}, comparison: {comparison_timestamp})"
                )

        logger.info(
            f"Detection complete for {generation_type}: {len(modified_cards)} modified, {len(unchanged_cards)} unchanged"
        )
        ...
        logger.error(f"Error detecting modified cards: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}
    ...
    try:
        ...
        return {
            ...
            "version": {
                ...
                "last_activity_at": version.last_activity_at.isoformat()
        ...
        logger.error(f"Error getting active report version: {e!s}", exc_info=True)
        return {"success": False, "error": str(e)}
    ...
    try:
        ...
        for report in reports:
            ...
            cards_stmt = (
                select(Card)
                .where(
                    Card.report_id == report.id, Card.is_active == True, Card.is_deleted == False
                )
                .order_by(Card.sequence)
            )
            ...
            report_data = {
                ...
                "last_activity_at": report.last_activity_at.isoformat()
...
async def insert_publish_details(
    publish_data: dict[str, Any], session: AsyncSession
) -> dict[str, Any]:
    """
...
async def get_report_domain_summary(user_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"DB error in get_report_domain_summary: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error in get_report_domain_summary: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}
...
async def get_reports_by_domain(
    ...
    try:
        ...
        return {
            ...
            "category_name": DOMAIN_DISPLAY_NAMES.get(
        ...
        logger.error(f"DB error in get_reports_by_domain: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}
    except Exception as e:
        logger.error(f"Unexpected error in get_reports_by_domain: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}


def _to_external_domain(internal_slug: str | None) -> str:
...
async def get_all_version_outputs_list(report_id: str, session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        if not versions:
            ...
            return {"success": False, "error": f"No versions found for report {report_id}"}
        ...
        for version in versions:
            ...
            if s3_uri.get("html") and (
                "html_explicit" not in s3_uri or s3_uri.get("html_explicit") == True
            ):
            ...
            if s3_uri.get("md") and (
                "md_explicit" not in s3_uri or s3_uri.get("md_explicit") == True
            ):
        ...
        return {"success": True, "report_id": report_id, "outputs": outputs}

    except Exception as e:
        logger.error(
            f"Error fetching version outputs list for report {report_id}: {e!s}", exc_info=True
        )
        return {"success": False, "error": str(e)}


async def get_version_file_for_download(
    report_id: str, version: int, file_type: str, session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        version_stmt = select(ReportVersion).where(
            ReportVersion.report_id == report_id, ReportVersion.version == version
        )
        ...
        if not report_version:
            ...
            return {"success": False, "error": f"Version {version} not found for this report"}
        ...
        logger.error(
            f"Error fetching {file_type} for report {report_id} version {version}: {e!s}",
            exc_info=True,
        )
        return {"success": False, "error": str(e)}


async def get_report_version_history_data(report_id: str, session: AsyncSession) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if active_version:
            ...
            s3_uri = active_version.s3_uri or {}
            has_any_output = (
                active_version.generated_at
                or s3_uri.get("md_generation_time")
                or s3_uri.get("html_generation_time")
                or s3_uri.get("info_pdf_generation_time")
                or s3_uri.get("pptx_generation_time")
            )
        ...
        for version in versions:
            ...
            version_data = {
                ...
                "generated_outputs": {
                    ...
                    "html": {
                        "generated": bool(s3_uri.get("html"))
                        and ("html_explicit" not in s3_uri or s3_uri.get("html_explicit") == True),
                        "s3_path": get_relative_path(s3_uri.get("html"))
                        if ("html_explicit" not in s3_uri or s3_uri.get("html_explicit") == True)
                        else None,
                        "generated_at": (
                            s3_uri.get("html_generation_time")
                            or (version.generated_at.isoformat() if version.generated_at else None)
                        )
                        if ("html_explicit" not in s3_uri or s3_uri.get("html_explicit") == True)
                        else None,
                    },
                    "md": {
                        "generated": bool(s3_uri.get("md"))
                        and ("md_explicit" not in s3_uri or s3_uri.get("md_explicit") == True),
                        "s3_path": get_relative_path(s3_uri.get("md"))
                        if ("md_explicit" not in s3_uri or s3_uri.get("md_explicit") == True)
                        else None,
                        "generated_at": (
                            s3_uri.get("md_generation_time")
                            or (version.generated_at.isoformat() if version.generated_at else None)
                        )
                        if ("md_explicit" not in s3_uri or s3_uri.get("md_explicit") == True)
                        else None,
        ...
        logger.info(
            f"Successfully retrieved {len(versions_data)} versions for report_id: {report_id}"
        )
        return {
            "success": True,
            "report_id": report_id,
...
async def get_report_info_by_version(
    report_id: str, report_version_id: str, session: AsyncSession
) -> dict[str, Any]:
    """
    ...
    if not report_id or not report_version_id:
        ...
        return {"success": False, "error": "Invalid report ID or version ID: empty value provided"}
    ...
    try:
        ...
        if not version:
            ...
            return {"success": False, "error": f"Version with ID {report_version_id} not found"}
        ...
        if not cards:
            logger.info(
                f"No cards in version snapshot, falling back to active cards for report {report_id}"
            )
            cards_stmt = (
                select(Card)
                .where(
                    Card.report_id == report_id, Card.is_active == True, Card.is_deleted == False
                )
                .order_by(Card.sequence)
            )
            cards_result = await session.execute(cards_stmt)
            ...
            cards_data.append(
                {
                    ...
                    if isinstance(card.content, dict)
                    else card.content or "",
                    "tables": card.content.get("tables", [])
                    if isinstance(card.content, dict)
                    else [],
                    "sub_sections": card.sub_sections,
                    "citations": card.citations,
        ...
        logger.info(
            f"Successfully retrieved report info for report_id: {report_id}, version_id: {report_version_id}"
        )
        return {"success": True, "reports": [report_data]}

    except SQLAlchemyError as e:
        logger.error(f"Database error retrieving report info: {e!s}", exc_info=True)
        return {"success": False, "error": f"Database error: {e!s}"}

    except Exception as e:
        logger.error(f"Unexpected error retrieving report info: {e!s}", exc_info=True)
        return {"success": False, "error": f"Unexpected error: {e!s}"}
