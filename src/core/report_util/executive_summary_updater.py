...
from src.config.constants import GEMINI_ES_MODEL_ID, OPENAI_API_KEY, PERPLEXITY_API_KEY, GEMINI_API_KEY, CARD_SUMMARY_MODEL
...
def check_if_update_required(
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            tool_choice={"type": "function", "function": {"name": "check_executive_summary_update_required"}}
        )
        save_raw_llm_response(
            response, CARD_SUMMARY_MODEL, "Checking whether the executive summary needs an update",
    ...
    try:
        ...
        save_raw_llm_response(
            interaction, GEMINI_ES_MODEL_ID, "Checking whether the executive summary needs an update (backup)",
            chat_id=chat_id, user_id=user_id,
        )
    ...
    while retry_count < max_retries:
        try:
            logger.info(f"Retrying Gemini API to check update requirement (attempt {retry_count + 1}/{max_retries})")
            gemini_client = genai.Client(api_key=GEMINI_API_KEY)
    ...
    # All APIs failed
    logger.error(f"All API attempts failed for update check")
    ...
    try:
        ...
        for idx, card in enumerate(cards, 1):
            ...
            if is_deleted:
                ...
                cards_info += f"**ACTION REQUIRED:** This section has been DELETED by the user.\n"
                cards_info += f"Remove ALL references to '{card_title}' from the executive summary.\n"
                cards_info += f"Ensure the executive summary flows naturally after removing this section.\n"
                cards_info += f"{'='*80}\n\n"
                ...
                cards_info += f"**CHANGES:** Compare the above two summaries to identify what changed.\n"
                cards_info += f"{'='*80}\n\n"
            else:
                # Handle unchanged cards
                cards_info += f"\n### Card {idx}: {card_title} (NOT REFINED)\n"
                cards_info += f"**Summary:**\n{current_summary}\n"
                cards_info += f"**Status:** This card was NOT refined - keep its information unchanged in ES.\n\n"
        
        logger.info(f"Found {refined_count} changed cards ({deleted_count} deleted) out of {len(cards)} total cards for report_id: {report_id}")
        
        # If no cards were refined or deleted, return the current executive summary unchanged
        if refined_count == 0:
            logger.info(f"No cards were refined or deleted, returning current executive summary unchanged for report_id: {report_id}")
        ...
        logger.info(f"Update required: {reasoning}. Proceeding with executive summary update... for report_id: {report_id}")
        ...
        # Try OpenAI first
        try:
            logger.info(f"Attempting to update executive summary using OpenAI API for report_id: {report_id}")
            client = OpenAI(api_key=OPENAI_API_KEY, timeout=120)
            ...
            save_raw_llm_response(
                response, CARD_SUMMARY_MODEL, "Updating the executive summary after report changes",
                chat_id=report_id, user_id=user_id,
            )
            
            result = json.loads(response.choices[0].message.tool_calls[0].function.arguments)
            updated_summary = result['updated_executive_summary']
            logger.info(f"Successfully updated executive summary with OpenAI API for report_id: {report_id}")
            return updated_summary
            
        except Exception as e:
            logger.error(f"Error with OpenAI API: {str(e)} for report_id: {report_id}")
            logger.info(f"Falling back to Gemini API for report_id: {report_id}")
        
        # Try Gemini as second fallback
        try:
            logger.info(f"Attempting to update executive summary using Gemini API for report_id: {report_id}")
            gemini_client = genai.Client(api_key=GEMINI_API_KEY)
            ...
            save_raw_llm_response(
                interaction, GEMINI_ES_MODEL_ID, "Updating the executive summary after report changes (backup)",
                chat_id=report_id, user_id=user_id,
            )
            
            result = json.loads(strip_json_code_fence(interaction.output_text))
            updated_summary = result['updated_executive_summary']
            logger.info(f"Successfully updated executive summary with Gemini API for report_id: {report_id}")
            return updated_summary
            
        except Exception as e:
            logger.error(f"Error with Gemini API: {str(e)} for report_id: {report_id}")
        ...
        while retry_count < max_retries:
            try:
                logger.info(f"Retrying Gemini API (attempt {retry_count + 1}/{max_retries}) for report_id: {report_id}")
                gemini_client = genai.Client(api_key=GEMINI_API_KEY)
                ...
                retry_count += 1
                logger.warning(f"Gemini API attempt {retry_count}/{max_retries} failed: {str(e)} for report_id: {report_id}")
                if retry_count < max_retries:
        ...
        logger.info(f"Returning original executive summary unchanged due to all API failures for report_id: {report_id}")
        return current_executive_summary
    
    except Exception as e:
        logger.critical(f"Unexpected error in update_executive_summary: {str(e)} for report_id: {report_id}")
        logger.info(f"Returning original executive summary unchanged due to unexpected error for report_id: {report_id}")
        return current_executive_summary
