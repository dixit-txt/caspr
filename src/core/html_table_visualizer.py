...
class HTMLInfoVisualization(BaseModel):
    """Structured output for infographic-style HTML visualization from any table."""
...
INFO_VISUALIZER_SYSTEM_PROMPT = """You are an expert information designer who transforms tabular data into creative, visually striking HTML infographics for PDF reports.
...
VISUAL FORMATS TO USE (pick the MOST CREATIVE one that fits the data):
...
5. **Visual Scorecard / Gauge Layout** — For ratings, confidence, performance data:
   ...
   - Circular gauges: div with border-radius: 50%, use border colors to show fill (e.g., 75% = 270deg of colored border)
...
CIRCLE/BUBBLE SIZING FORMULA:
- Find the maximum numeric value in the data
- Largest item: 180-200px diameter
- Others: scale proportionally with minimum of 60px so text fits
- If no clear numeric values exist, size by text importance or use varied sizes with color differentiation
- For very large datasets (8+ items), use smaller max (140px) and min (50px)

STRICT COLOR PALETTE — Use ONLY these 7 colors. No exceptions. No other hex values allowed:
...
NEVER USE ANY OTHER COLORS. No greens, blues, purples, ambers, teals, pinks, or any color outside this palette.
...
NO SVG:
- Do NOT use <svg> tags. Use CSS-only shapes (border-radius for circles, borders for lines, transform for rotation).
...
INFO_VALIDATION_SYSTEM_PROMPT = """You are an expert HTML/CSS validator for PDF-rendered infographic layouts.

Your task: Check if the provided HTML infographic will render correctly when converted to PDF.

CHECK FOR THESE ISSUES:

1. **CRITICAL — No Tables**: The output must NOT contain <table>, <tr>, <td>, <th>, <thead>, <tbody> tags. If found, mark as critical.
...
6. **CRITICAL — Overflow**: Outer container must have max-width: 620-700px with box-sizing: border-box and `overflow-x: hidden` (X axis only). Flag as critical if the outer container uses `overflow: hidden` on BOTH axes or has any fixed/max height combined with `overflow: hidden` around text — that clips content vertically ("visual not fully generated"). Text must wrap (overflow-wrap: break-word).

7. **CRITICAL — Font Compliance**: Only 'Calibri', 'Baskerville', 'Inter', 'DejaVu Serif' allowed as primary fonts.

8. **Data Integrity**: ALL rows and columns from the original data must be represented in the visual.
...
10. **CRITICAL — Height Fits One Page (WITHOUT CLIPPING)**: The total rendered height of the visualization MUST NOT exceed ~700px. If it would exceed that (too many items, oversized shapes, tall vertical stacks), flag as critical and instruct making it more compact (smaller shapes, less spacing, fewer/aggregated items). BUT this must be achieved by compact design, NOT by clipping — see item 16.

11. **Visual Quality**: Readable text, adequate contrast, elements not clipped or overflowing, positioned elements within container bounds.
...
14. **CRITICAL — No Fixed Heights / No Vertical Clipping**: Flag as critical if any text-containing container/card/shape has a fixed `height` (e.g. `height: 480px`) or a `max-height` combined with `overflow: hidden`, because text that exceeds it gets cut off ("visualization not completely generated", "text missing beneath the visual"). Text containers must use padding with auto/min-height so they grow to fit. Instruct removing the fixed height and any vertical `overflow: hidden`.

15. **CRITICAL — No transform: scale()**: Flag as critical if `transform: scale(...)` is used to size the visual. A CSS scale leaves the original layout box size, creating a large empty gap after the visual / a blank page. Instruct sizing at 1:1 instead.

16. **Hard Width Enforcement**: No element at any nesting level should render wider than 700px. Check for fixed widths > 700px, percentage widths on containers wider than 700px, or flex/grid layouts that could push content beyond 700px total width.
...
def _invoke_gemini_info(
    user_prompt: str,
    chat_id: str | None = None,
    user_id: str | None = None,
    *,
    log_context: str = "Building an interactive table visualization (backup)",
) -> HTMLInfoVisualization:
    interaction = GEMINI_CLIENT.interactions.create(
        model=GEMINI_HTML_MODEL,
        input=user_prompt,
        system_instruction=INFO_VISUALIZER_SYSTEM_PROMPT,
        response_format={
            "type": "text",
            "mime_type": "application/json",
            "schema": _GEMINI_INFO_SCHEMA,
        },
        generation_config={
            "temperature": 0.3,
...
def _invoke_anthropic_viz_info(
    user_prompt: str,
    chat_id: str | None = None,
    user_id: str | None = None,
    *,
    log_context: str = "Building an interactive table visualization",
) -> HTMLInfoVisualization:
    messages = [
        _cached_system_message(INFO_VISUALIZER_SYSTEM_PROMPT),
        HumanMessage(content=user_prompt),
    ]
    structured_llm = ANTHROPIC_VIZ_LLM.with_structured_output(HTMLInfoVisualization, include_raw=True)
    ...
    result: HTMLInfoVisualization = raw_result["parsed"]
    if not result or not result.html_code:
        raise ValueError("Model returned empty structured output")
    return result


def _validate_info_html(html_code: str, table_data: str, chat_id: str | None = None, user_id: str | None = None) -> HTMLValidationResult:
    """
    ...
    if table_tags:
        return HTMLValidationResult(
            ...
            severity="critical",
            fix_instructions="Replace all <table>/<tr>/<td> with div-based infographic layouts (bubbles, cards, flows).",
        )
...
Check rendering safety, layout validity, data integrity, and visual quality."""
    ...
    try:
        response = SYNC_OPENAI_CLIENT.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": INFO_VALIDATION_SYSTEM_PROMPT},
                {"role": "user", "content": validation_prompt},
            ],
        )
        save_raw_llm_response(response, OPENAI_VALIDATION_MODEL, "Checking that a table visualization looks correct", chat_id, user_id=user_id)
        ...
        try:
            interaction = GEMINI_CLIENT.interactions.create(
                ...
                system_instruction=INFO_VALIDATION_SYSTEM_PROMPT,
                response_format={
                    "type": "text",
                    "mime_type": "application/json",
            ...
            save_raw_llm_response(interaction, GEMINI_VALIDATION_MODEL, "Checking that a table visualization looks correct (backup)", chat_id, user_id=user_id)
...
def _invoke_info_with_fallback(
    user_prompt: str,
    chat_id: str | None = None,
    user_id: str | None = None,
) -> HTMLInfoVisualization | None:
    """Try Anthropic Sonnet 4.6 (primary) -> Gemini 3.1 Pro fallback on Sonnet failure."""
    try:
        logger.info(f"[InfoViz] Trying Anthropic Sonnet 4.6 ({ANTHROPIC_MODEL_ID})")
        result = _invoke_anthropic_viz_info(user_prompt, chat_id, user_id)
        logger.info(f"[InfoViz] Anthropic Sonnet 4.6 succeeded: {result.visualization_type}")
        return result
    except Exception as e:
        logger.error(f"[InfoViz] Anthropic Sonnet 4.6 failed: {e}")

    try:
        logger.info(f"[InfoViz] Trying Gemini ({GEMINI_HTML_MODEL})")
        result = _invoke_gemini_info(user_prompt, chat_id, user_id)
        logger.info(f"[InfoViz] Gemini succeeded: {result.visualization_type}")
        return result
    except Exception as e:
        logger.error(f"[InfoViz] Gemini also failed: {e}")
        return None


def generate_info_visualization(table_data: str, user_refine_prompt: str = None, chat_id: str | None = None, user_id: str | None = None) -> HTMLInfoVisualization | None:
    """
    Generate a creative infographic-style HTML visualization from any table data.

    Transforms tables into bubble charts, timelines, mosaics, gauges, or other
    visual formats — NEVER into another table.
    ...
    Returns:
        HTMLInfoVisualization with html_code, visualization_type, and reasoning.
        None if all providers fail.
    """
    user_instruction = ""
    if user_refine_prompt:
        user_instruction = f"\n\nUSER INSTRUCTION: {user_refine_prompt}\nFollow the user's instruction for the visualization style."

    prompt_text = f"""INPUT TABLE DATA:
{table_data}{user_instruction}

Transform this table into a CREATIVE visual infographic (bubble chart, timeline, mosaic, gauges, etc. — NOT a table). You MUST provide all three fields: html_code, visualization_type, and reasoning."""

    visualization = _invoke_info_with_fallback(prompt_text, chat_id=chat_id, user_id=user_id)

    if visualization is None:
        return None

    for attempt in range(MAX_VALIDATION_RETRIES):
        logger.info(f"[InfoViz] Validating HTML (attempt {attempt + 1}/{MAX_VALIDATION_RETRIES})")
        validation = _validate_info_html(visualization.html_code, table_data, chat_id=chat_id, user_id=user_id)

        if validation.is_valid or validation.severity == "pass":
            logger.info("[InfoViz] HTML validation passed")
            visualization.html_code = _sanitize_info_html(visualization.html_code)
            visualization.html_code = _ensure_page_break_avoid(visualization.html_code)
            return visualization

        logger.warning(f"[InfoViz] Validation failed ({validation.severity}): {validation.issues}")
...
Generate a corrected CREATIVE infographic visualization (NOT a table). Use bubbles, flows, mosaics, or gauges. Ensure PDF safety."""

        try:
            regen_result = _invoke_info_with_fallback(
                fix_user_prompt,
                chat_id=chat_id,
                user_id=user_id,
            )
            if regen_result is None:
                logger.error(f"[InfoViz] Regeneration attempt {attempt + 1} failed")
                break
            visualization = regen_result
        except Exception as e:
            logger.error(f"[InfoViz] Regeneration attempt {attempt + 1} failed: {e}")
            break
...
def _sanitize_info_html(html_code: str) -> str:
    """
    Strip HTML comments, collapse empty lines, fix overflow issues,
    remove any sneaky table tags, and strip citations/links.
    """
