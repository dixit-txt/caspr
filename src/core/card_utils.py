...
import markdown
import pandas as pd
from pandasai.safe_libs.restricted_matplotlib import plt
from src.core.dynamic_prompting import decide_if_table_needs_visualization
...
from src.config.constants import CARD_SUMMARY_MODEL, GEMINI_DRL_GEN_MODEL, GEMINI_SUMMARY_MODEL, OPENAI_LLM, S3_REPORTS_BASE_PATH, GEMINI_API_KEY, GEMINI_ES_MODEL_ID
from src.core.html_graph_maker import generate_html_visualization
from src.core.html_table_visualizer import generate_info_visualization
from src.core.prompt_utils import CARD_GEN_JSON_SCHEMA, build_card_gen_prompt, CARD_SUMMARY_PROMPT, CARD_SUMMARY_SCHEMA, CARD_SUMMARY_SCHEMA_GEMINI, DRL_JSON_SCHEMA_OPENAI, DRL_JSON_SCHEMA_GEMINI, DRL_PROMPT, MERGE_CUMULATIVE_SUMMARY_PROMPT, MERGE_CUMULATIVE_SUMMARY_SCHEMA, MERGE_CUMULATIVE_SUMMARY_SCHEMA_GEMINI, NAPKIN_AI_TABLE_TO_VIZ_PROMPT, REFINE_REPORT_LAYOUT_PROMPT, REFINE_REPORT_LAYOUT_SCHEMA, SECTION_OR_SUBSECTION_SUMMARY_PROMPT
from src.core.prompt_utils import REFINE_CUMULATIVE_SUMMARY_SCHEMA, REFINE_CUMULATIVE_SUMMARY_SCHEMA_GEMINI, REFINE_CUMULATIVE_SUMMARY_PROMPT
from src.core.prompt_utils import COMPRESS_CONTEXT_SUMMARY_PROMPT, COMPRESS_CONTEXT_SUMMARY_SCHEMA, COMPRESS_CONTEXT_SUMMARY_SCHEMA_GEMINI
# from src.config.constants import LLM
from src.core.card_fixer import fix_card
from src.core.s3_utils import get_s3_instance
from urllib.parse import urlparse, parse_qs, urlencode, urlunparse
from src.config.constants import ANTHROPIC_LLM, ANTHROPIC_MODEL_ID
...
from src.core.grep_agent_2 import ask_pdfs as grep_ask_pdfs, PdfSession
from src.core.prompt_utils import TABLE_TO_VIZ_PROMPT, PANDASAI_VIZ_REFINE_PROMPT
from src.core.viz_pipeline import generate_mermaid_viz
from src.core.citation_utils import generate_citation_url
from typing import Optional, Dict
import base64
import os
from src.core.s3_utils import S3_boto3_client
from PIL import Image
import io
from dotenv import load_dotenv
from src.config.constants import ENVIRONMENT
from src.config.constants import VISUALIZATION_MODEL, SYNC_OPENAI_CLIENT
import urllib.parse
from src.config.constants import OPENAI_API_KEY
from src.config.constants import DRL_GEN_MODEL
from src.config.constants import COMPRESS_CONTEXT_SUMMARY_MODEL
from src.config.constants import CARD_GEN_MODEL_OPENAI
from src.config.constants import PRIORITIZE_ARXIV
from src.config.constants import GENERATE_TITLE_FOR_TABLE_MODEL
from src.config.constants import PLOT_TYPE_MODEL
from src.config.constants import REFINE_REPORT_LAYOUT_MODEL
from src.config.constants import GEMINI_COMPRESS_CONTEXT_SUMMARY_MODEL, GEMINI_GENERATE_TITLE_FOR_TABLE_MODEL, GEMINI_PLOT_TYPE_MODEL, GEMINI_BRIEF_STREAM_MODEL
...
def image_to_base64_s3(image_path: str):
    ...
    try:
        ...
        logger.error(f"Failed to convert S3 image to base64: {str(e)}")
        raise


class SubSection(BaseModel):
    name: str = Field(..., description="Plain text sub-section title without any markdown symbols. e.g., 'Overview', 'Key Findings'. Do NOT include '#', '##', '###', bullets, or numbering.")
    content: str = Field(
        ...,
        description="Detailed markdown content for this sub-section. Include inline citations like [1], [2] where facts/stats are used."
    )

class SectionOutput(BaseModel):
    section: str = Field(..., description="Plain text section name without any markdown symbols. e.g., 'Market Analysis', 'Industry Overview'. Do NOT include '#', '##', bullets, or numbering.")
    content: str = Field(..., description="This should be the guidance about what to include here about the above section")
...
class SubSectionLayout(BaseModel):
    ...
    description: str = Field(..., description="Detailed description of what this sub-section should contain")

class SectionLayout(BaseModel):
    section: str = Field(..., description="Section title in markdown format, e.g., '## ABCD'")
    sub_sections: List[SubSectionLayout] = Field(default_factory=list, description="List of sub-sections within this section")
...
def generate_drl(report_layout: str, user_instructions: str, report_type: str = "study", max_retries=3, retry_delay=5, latest_info_context: str = None, chat_id: str = None, user_id: str = None) -> dict:
    """
    ...
    if latest_info_context:
        ...
        logger.info(f"Augmented user instructions with latest_info_context ({len(latest_info_context)} chars)")
    ...
    while regeneration_attempt < max_regeneration_attempts:
        try:
            if regeneration_attempt > 0:
                logger.info(f"Regeneration attempt {regeneration_attempt}/{max_regeneration_attempts - 1} for DRL due to section count mismatch")
            
            logger.info("Attempting to generate DRL using OpenAI API")
            response = SYNC_OPENAI_CLIENT.chat.completions.create(
                model=DRL_GEN_MODEL,
                messages=[{"role": "user", "content": DRL_PROMPT.format(report_layout=report_layout, user_instructions=augmented_instructions, report_type=report_type.upper(), current_date=datetime.datetime.now().strftime('%B %d, %Y'))}],
                tools=[DRL_JSON_SCHEMA_OPENAI],
                tool_choice={"type": "function", "function": {"name": "generate_descriptive_report_layout"}}
            )
            save_raw_llm_response(response, DRL_GEN_MODEL, "Creating the detailed report outline", chat_id, user_id=user_id)
            ...
            if len(filtered_drl) < len(drl):
                logger.warning(f"Removed {len(drl) - len(filtered_drl)} sections with empty section names from DRL")
            ...
            for section in filtered_drl:
                if 'sub_sections' in section:
                    ...
                    section['sub_sections'] = [
                        sub_section for sub_section in section['sub_sections'] 
                        if sub_section.get('name', '').strip()
                    ]
                    filtered_sub_count = len(section['sub_sections'])
                    if filtered_sub_count < original_sub_count:
                        logger.warning(f"Removed {original_sub_count - filtered_sub_count} subsections with empty names from section: {section.get('section', 'Unknown')}")
            ...
            if actual_section_count < expected_section_count:
                logger.warning(f"DRL has fewer sections ({actual_section_count}) than report_layout ({expected_section_count})")
                regeneration_attempt += 1
                if regeneration_attempt < max_regeneration_attempts:
                    logger.info(f"Will regenerate DRL (attempt {regeneration_attempt + 1}/{max_regeneration_attempts})")
                    time.sleep(2)
                    continue
                else:
                    logger.warning(f"Max regeneration attempts reached. Proceeding with DRL containing {actual_section_count} sections")
                    return filtered_drl
            else:
                logger.info(f"DRL section count matches or exceeds expected count. Proceeding with {actual_section_count} sections")
                return filtered_drl
            ...
            while retry_count < max_retries:
                try:
                    ...
                    drl_prompt = DRL_PROMPT.format(report_layout=report_layout, user_instructions=augmented_instructions, report_type=report_type.upper(), current_date=datetime.datetime.now().strftime('%B %d, %Y')).strip()
                    ...
                    save_raw_llm_response(interaction, GEMINI_DRL_GEN_MODEL, "Creating the detailed report outline (backup)", chat_id, user_id=user_id)
                    ...
                    if actual_section_count < expected_section_count:
                        ...
                        regeneration_attempt += 1
                        if regeneration_attempt < max_regeneration_attempts:
                            logger.info(f"Will regenerate DRL using Gemini (attempt {regeneration_attempt + 1}/{max_regeneration_attempts})")
                    ...
                    retry_count += 1
                    logger.warning(f"Gemini API attempt {retry_count}/{max_retries} failed: {str(e)}")
                    if retry_count < max_retries:
                        logger.info(f"Retrying in {retry_delay} seconds...")
                        time.sleep(retry_delay)
                    else:
                        logger.critical(f"All Gemini API retry attempts failed: {str(e)}")
                        return {}
    ...
    try:
        ...
        for section_layout in clean_drl[1:]:
            ...
            cleaned_rl.append({
                ...
                'sub_sections': [sub_section['name'] for sub_section in section_layout['sub_sections']]
            })
        ...
        logger.error(f"Error cleaning DRL to Rl: {str(e)}")
        return []
...
def generate_report_layout(drl):
    ...
    try:
        ...
        logger.error(f"Error generating report layout: {str(e)}")
        return []
...
def compress_context_summary(raw_summaries: str, max_retries: int = 3, retry_delay: int = 5, chat_id: str = None, user_id: str = None) -> str:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            save_raw_llm_response(response, COMPRESS_CONTEXT_SUMMARY_MODEL, "Condensing earlier section summaries for later writing", chat_id, user_id=user_id)
            result = json.loads(response.choices[0].message.tool_calls[0].function.arguments)
            compressed = result["compressed_summary"]
            logger.info(f"Context summary compressed: {len(raw_summaries)} chars -> {len(compressed)} chars")
            return compressed
        except Exception as e:
    ...
    logger.info("OpenAI attempts exhausted for compress_context_summary, falling back to Gemini API")
    try:
        gemini_client = genai.Client(api_key=GEMINI_API_KEY)
        interaction = gemini_client.interactions.create(
        ...
        save_raw_llm_response(interaction, GEMINI_COMPRESS_CONTEXT_SUMMARY_MODEL, "Condensing earlier section summaries for later writing (backup)", chat_id, user_id=user_id)
        result = json.loads(strip_json_code_fence(interaction.output_text))
        compressed = result["compressed_summary"]
        logger.info(f"Context summary compressed via Gemini fallback: {len(raw_summaries)} chars -> {len(compressed)} chars")
        return compressed
    except Exception as e:
...
def generate_cards(card_layout, 
    cumulative_summary="", 
    ...
    grep_session:Optional[PdfSession]=None,
    chat_id: str = None,
    user_id: str = None,
    analytics_collector: Optional[List[Dict]] = None,
    analytics_operation_id: Optional[str] = None,
    ...
    logger.info(f"Built prompt for section '{target_section}' (first={is_first_section}, has_file={bool(upload_file_config)}, has_grep_session={bool(grep_session)}, prioritize_arxiv={prioritize_arxiv})")
    ...
    for openai_attempt in range(1, openai_max_retries + 1):
        try:
            # raise Exception("test")
            logger.info(f"Sending request to OpenAI API (attempt {openai_attempt}/{openai_max_retries})")

            # Case 1: Web search only (no file)
            if web_search and not upload_file_config:
                logger.info(f"Generating cards for section: {target_section} with web search only (prioritize_arxiv={prioritize_arxiv})")
                ...
                save_raw_llm_response(response, CARD_GEN_MODEL_OPENAI, "Writing report section content using web research", chat_id, user_id=user_id)

            # Case 2: Web search + Vector Store
            elif web_search and vector_store_id:
                logger.info(f"Generating cards for section: {target_section} with web search and vector store only")
                ...
                save_raw_llm_response(response, CARD_GEN_MODEL_OPENAI, "Writing report section content using uploaded documents and web research", chat_id, user_id=user_id)
            ...
            # Case 4: Vector Store only (no web search)            
                ...
                logger.info(f"Generating cards for section: {target_section} with vector store only {vector_store_id} with file_id {file_ids} without web search")
                ...
                save_raw_llm_response(response, CARD_GEN_MODEL_OPENAI, "Writing report section content from uploaded documents", chat_id, user_id=user_id)
            ...
            # Web search is added alongside if enabled.
                ...
                def _grep_tool_handler(args: dict) -> str:
                    ...
                    try:
                        result = grep_ask_pdfs(grep_session, question, chat_id=chat_id, user_id=user_id)
                ...
                for _round in range(max_tool_rounds):
                    ...
                    save_raw_llm_response(response, CARD_GEN_MODEL_OPENAI, "Writing report section content by searching your documents", chat_id, user_id=user_id)
                    ...
                    logger.info(f"[grep_agent_2] Round {_round + 1} — resolved {len(fn_calls)} search_documents call(s)")
            ...
            # matching one of the stricter cases above.
                ...
                save_raw_llm_response(response, CARD_GEN_MODEL_OPENAI, "Writing report section content (plain generation)", chat_id, user_id=user_id)

            # Case 6: External tools (e.g. dd_research) + optional web search + file_search
            if external_tools:
                logger.info(f"Generating cards for section: {target_section} with external tools only")
                ext_tool_handlers = {}
                api_tools = []
                ...
                for _round in range(max_tool_rounds):
                    ...
                    save_raw_llm_response(response, CARD_GEN_MODEL_OPENAI, "Writing report section content using research tools", chat_id, user_id=user_id)
                    ...
                    for fc in fn_calls:
                        ...
                        logger.info(f"[external_tools] Round {_round+1} — model called {fn_name}({json.dumps(fn_args, default=str)[:200]})")
                        ...
                        if handler:
                            ...
                            result_text = json.dumps({"error": f"No handler for function: {fn_name}"})
                    ...
                    logger.info(f"[external_tools] Round {_round+1} — resolved {len(fn_calls)} function call(s)")
            ...
            # raise Exception("test")
            logger.error(f"OpenAI API attempt {openai_attempt}/{openai_max_retries} failed: {str(e)}")   
            if _is_context_length_error(e) and not _used_context_fallback and previous_cards:
                ...
                cumulative_summary_fallback = _build_cumulative_summary_from_cards(
                    previous_cards, chat_id=chat_id, user_id=user_id,
                )
                prompt = build_card_gen_prompt(
                    target_section=target_section,
                    section_description=section_description,
            ...
            if openai_attempt < openai_max_retries:
                ...
                logger.error(f"All {openai_max_retries} OpenAI API attempts failed, falling back to Gemini API")
...
def gather_context_from_uploaded_file(
    ...
    logger.info(f"[gather_context_from_uploaded_file] Gathering context for {len(drl)} sections in parallel")
    ...
    logger.info(f"[gather_context_from_uploaded_file] Done — {total_chars} total chars gathered across {len(tasks)} queries")
...
class BriefOutput(BaseModel):
    sections: List[BriefSectionOutput] = Field(..., description="Surface-level overview report sections (flat, no sub-sections)")
...
def _stream_brief_chunks(
    ...
    """
    from src.core.prompt_utils import BRIEF_SYSTEM_PROMPT, BRIEF_ARXIV_PRIORITY_ADDENDUM, build_brief_prompt

    current_date = datetime.datetime.now().strftime('%B %d, %Y')
    prompt = build_brief_prompt(drl, user_instructions, current_date)

    system_msg = BRIEF_SYSTEM_PROMPT.format(current_date=current_date)
    call_kwargs = {}
    if prioritize_arxiv:
    ...
    logger.info(f"[brief_stream] Starting stream — {CARD_GEN_MODEL_OPENAI} + web_search + BriefOutput schema, DRL has {len(drl)} sections, prioritize_arxiv={prioritize_arxiv}")
    ...
    try:
        ...
        with stream as s:
            ...
            if final_response is not None:
                try:
                    save_raw_llm_response(final_response, CARD_GEN_MODEL_OPENAI, "Streaming a short research preview", chat_id, user_id=user_id)
                except Exception:
                    pass
        ...
        if yielded_any_chunk:
            ...
            logger.error(f"[brief_stream] OpenAI stream failed mid-output, cannot fall back to Gemini: {e}")
            raise
        logger.error(f"[brief_stream] OpenAI stream failed before any output ({e}), falling back to Gemini API")
    ...
    try:
        save_raw_llm_response(interaction, GEMINI_BRIEF_STREAM_MODEL, "Streaming a short research preview (backup)", chat_id, user_id=user_id)
    except Exception:
        pass

    try:
        collect_gemini_search_analytics(
            interaction, analytics_collector, analytics_operation_id, model_used=GEMINI_BRIEF_STREAM_MODEL
        )
    except Exception:
        pass
...
def _parse_brief_stream(chunk_generator) -> Generator[dict, None, None]:
    """
...
def generate_brief_cards(
    ...
    """
    logger.info(f"[generate_brief_cards] Starting brief generation — {len(drl)} sections, prioritize_arxiv={prioritize_arxiv}")
...
def replace_brief_citations(card: dict, citation_url_map: dict, all_report_citations: list) -> dict:
    """Renumber inline citations in a brief card to use global sequential numbers.

    The brief LLM produces citations as [N](url) with per-section numbering.
    This function rewrites them to [global_number](cleaned_url) so numbering
    is consistent across the entire report, and populates card["citations"].
    ...
    Returns:
        The mutated card with renumbered inline citations and populated citations dict.
    """
    card_citation_map = {}
...
def replace_citations(text, citation_indices, processed_citations, citation_url_map,report_citations, url_to_snippet: Optional[Dict[str, str]] = None):
    try:
        ...
        def fix_broken_urls(text):
            ...
            broken_url_pattern = r'(/(?:www\.|[a-zA-Z0-9-]+\.)[a-zA-Z0-9.-]+(?:/[^\s)]*)?)\s*\n\s*([^\s)]+)'
            ...
            domain_broken_pattern = r'((?:www\.|[a-zA-Z0-9-]+\.)[a-zA-Z0-9.-]+(?:/[^\s)]*)?)\s*\n\s*([^\s)]+)'
        ...
        def malformed_citation_replacer(match):
            ...
            if url_match:
                ...
                if url in citation_url_map:
                    global_index = citation_url_map[url]
                    logger.info(f"Found existing citation for malformed URL: [{global_index}]({url})")
                else:
                    # Assign new citation number
                    report_citations.append(url)
                    global_index = len(report_citations)
        ...
        def descriptive_citation_replacer(match):
            ...
            if url_match:
                ...
                if url in citation_url_map:
                    global_index = citation_url_map[url]
                    logger.info(f"Found existing citation for descriptive URL: [{global_index}]({url})")
                else:
                    # Assign new citation number
                    report_citations.append(url)
                    global_index = len(report_citations)
                    citation_url_map[url] = global_index
                    logger.info(f"Created new citation for descriptive URL: [{global_index}]({url})")
        ...
        updated_text = re.sub(descriptive_citation_pattern, descriptive_citation_replacer, updated_text)
        ...
        logger.error(f"Error replacing citations: {str(e)}")
        return ""   

def generate_section_summary(section_content, chat_id: str = None, user_id: str = None):    
    formatted_summary_prompt = CARD_SUMMARY_PROMPT.format(
    ...
    try:
        ...
        response = SYNC_OPENAI_CLIENT.chat.completions.create(
            ...
            tool_choice={"type": "function", "function": {"name": "generate_report_section_summary"}}
        )
        save_raw_llm_response(response, CARD_SUMMARY_MODEL, "Summarizing a report section", chat_id, user_id=user_id)
        result = json.loads(response.choices[0].message.tool_calls[0].function.arguments)
        ...
        save_raw_llm_response(interaction, GEMINI_SUMMARY_MODEL, "Summarizing a report section (backup)", chat_id, user_id=user_id)
...
def generate_summary_for_ask_caspr(section_content: str, citations_list: list, chat_id: str = None, user_id: str = None)->str:
    """
    ...
    try:
        ...
        save_raw_llm_response(response, CARD_SUMMARY_MODEL, "Creating a summary for Ask Caspr", chat_id, user_id=user_id)
        result = json.loads(response.choices[0].message.tool_calls[0].function.arguments)
        summary = result['summary']
        logger.info("Successfully generated section summary")
        return summary
        ...
        save_raw_llm_response(interaction, GEMINI_SUMMARY_MODEL, "Creating a summary for Ask Caspr (backup)", chat_id, user_id=user_id)
...
def update_summaries_for_ask_caspr(card: dict, chat_id: str = None, user_id: str = None)->dict:
    ...
    try:
        ...
        if not card['section'][0]['content'] or card['section'][0]['content'].strip() == '':
            ...
            section_summary = generate_summary_for_ask_caspr(card['section'][0]['content'],citations, chat_id=chat_id, user_id=user_id)
        ...
        for sub_section in card['sub_sections']:
            if not sub_section['content'] or sub_section['content'].strip() == '':
                ...
                sub_section_summary = generate_summary_for_ask_caspr(sub_section['content'],citations, chat_id=chat_id, user_id=user_id)
...
def generate_cumulative_summary(previous_cumulative_summary, new_section_summary, chat_id: str = None, user_id: str = None):
    try:
        ...
        save_raw_llm_response(response, CARD_SUMMARY_MODEL, "Building the overall report summary", chat_id, user_id=user_id)
        ...
        save_raw_llm_response(interaction, GEMINI_ES_MODEL_ID, "Building the overall report summary (backup)", chat_id, user_id=user_id)
...
def refine_cumulative_summary(cards_for_db: list, max_retries: int = 3, retry_delay: int = 5, chat_id: str = None, user_id: str = None) -> str:
    """
    ...
    try:
        ...
        while retry_count < max_retries:
            try:
                logger.info(f"OpenAI API attempt {retry_count + 1}/{max_retries} for executive summary")
                response = SYNC_OPENAI_CLIENT.chat.completions.create(
                    model=CARD_SUMMARY_MODEL,
                ...
                save_raw_llm_response(response, CARD_SUMMARY_MODEL, "Polishing the executive summary", chat_id, user_id=user_id)
                result = json.loads(response.choices[0].message.tool_calls[0].function.arguments)
                ...
                if retry_count < max_retries:
                    ...
                    raise Exception(f"OpenAI failed after {max_retries} retries: {str(e)}")
        
    except Exception as e:
        logger.error(f"OpenAI failed: {str(e)}, falling back to Gemini")
        ...
        while retry_count < max_retries:
            try:
                ...
                save_raw_llm_response(interaction, GEMINI_ES_MODEL_ID, "Polishing the executive summary (backup)", chat_id, user_id=user_id)
...
def _download_html_from_s3(s3_path: str) -> str:
    ...
    try:
        ...
        logger.error(f"Failed to download HTML from S3: {s3_path} - {str(e)}")
        return None
...
def convert_json_to_md(complete_report_json, table_visualization_map=None):
    ...
    for index, item in enumerate(complete_report_json):
        ...
        if "section" in item and isinstance(item["section"], list) and item["section"]:
            ...
            if content:
                if after_exec_summary and sec_tables and table_visualization_map:
                    ...
                    for t in sec_tables:
                        ...
                        logger.info(f"Resolved visualization for markdown")
            ...
            for sub_idx, sub in enumerate(item.get("sub_sections", [])):
                ...
                if sub_content:
                    if after_exec_summary and sub_tables and table_visualization_map:
                        ...
                        for t in sub_tables:
                            ...
                            if pos != -1:
                                insert_at = pos + len(table_md)
                                sub_content = sub_content[:insert_at] + "\n\n" + viz + "\n" + sub_content[insert_at:]
                                handled_ids.add(tid)
                ...
                if after_exec_summary and not sub_content and sub_tables and table_visualization_map:
                    for t in sub_tables:
                        tid = (t.get("table_id") or "").strip()
                        viz = (t.get("visualization") or "").strip()
                        viz = _resolve_viz_for_md(viz)
    ...
    # [number](url). Upstream passes normally number citations, but an LLM (e.g.
    # the card fixer) or the executive summary can reintroduce a URL/text label,
    # producing an ugly [url](url). Rewrite any non-numeric label to its citation
    # number, preferring the authoritative per-card citation map so numbering stays
    # consistent with the report's sources list.
    final_md = _enforce_numbered_citations(final_md, complete_report_json)
    return final_md
...
def cards_for_db_to_cards_for_fe(card):
    ...
    updated_content = re.sub(citation_pattern, lambda match: f"[{new_citations[match.group(2)]}]({match.group(2)})", content)
...
def generate_title_for_table(table, chat_id: str = None, user_id: str = None):
    ...
    try:
        ...
        save_raw_llm_response(title["raw"], GENERATE_TITLE_FOR_TABLE_MODEL, "Creating a title for a table", chat_id, user_id=user_id)
        ...
        try:
            ...
            save_raw_llm_response(interaction, GEMINI_GENERATE_TITLE_FOR_TABLE_MODEL, "Creating a title for a table (backup)", chat_id, user_id=user_id)
            result = json.loads(strip_json_code_fence(interaction.output_text))
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Gemini fallback for choose the best plot type also failed: {gemini_e}")
                return None

def _parse_table_to_dataframe(table: str) -> Optional[pd.DataFrame]:
    try:
        ...
        logger.warning(f"Unable to parse table as dataframe: {str(e)}")
        return None
...
def _is_small_table(table: str) -> bool:
    ...
    logger.info(f"Parsed table shape before visualization decision: rows={row_count}, columns={column_count}")
    return row_count <= 3 and column_count <= 2


def generate_table_visualization(table: str, chat_id: str, aspect_ratio: Optional[str] = "4:3", max_retries: int = 6, report_type: str = "study", user_id: str = None):
    """Generate table visualization with decision-based HTML first, then Gemini/PandasAI fallback.
    ...
    Flow:
        ...
        6. If Gemini fails -> PandasAI fallback
    ...
    if is_quantitative:
        try:
            ...
            logger.error(f"HTML graph visualization failed: {str(e)}")
    ...
    try:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                save_raw_llm_response(interaction, VISUALIZATION_MODEL, "Creating a chart image from table data", chat_id, user_id=user_id)
    ...
    # Step 4: Fallback to PandasAI
    try:
        logger.info("Falling back to PandasAI for visualization")
        formatted_viz_prompt = PANDASAI_VIZ_REFINE_PROMPT.format(table=table)
        # Brief cards can post-process in parallel, so each visualization needs
        # its own temp directory instead of sharing one per chat.
        dest_dir = f"temp_viz_dir_{chat_id}_{uuid7()}"
        os.makedirs(dest_dir, exist_ok=True)
        before_images = set(os.listdir(dest_dir))
        ...
        smart_df = SmartDataframe(
            ...
            config={
                "llm": OPENAI_LLM,
                "save_charts_path": dest_dir,
                "save_charts": True,
                "open_charts": False,
                "enable_cache": False,
                "data_viz_library": "matplotlib",
                "max_retries": 5,
                "retry_delay": 5
            }
        )
        ...
        while retry_count < max_retries:
            try:
                response = smart_df.chat(formatted_viz_prompt)
                logger.info(f"PandasAI Response: {response}")
                break
            except Exception as e:
                retry_count += 1
                logger.warning(f"PandasAI attempt {retry_count}/{max_retries} failed: {str(e)}")
                if retry_count >= max_retries:
                    logger.error(f"All {max_retries} PandasAI attempts failed")
                    raise
                time.sleep(1)

        after_images = set(os.listdir(dest_dir))
        new_images = list(after_images - before_images)
        if new_images:
            image_path = os.path.join(dest_dir, new_images[0])
            return image_path

    except Exception as e:
        logger.error(f"PandasAI visualization failed: {str(e)}")

    return None

def generate_visualization(table: str, user_name: str, chat_id: str, report_type: str = "study", user_id: str = None):
    ...
    try:  
        image_path = generate_table_visualization(table, chat_id, report_type=report_type, user_id=user_id)
        ...
        if image_path:
            ...
            if image_dir and os.path.basename(image_dir).startswith(f"temp_viz_dir_{chat_id}_") and os.path.exists(image_dir):
...
def add_viz_to_card(card, user_name, chat_id, report_type: str = "study", user_id: str = None):
    ...
    """
    logger.info(f"Adding visualizations to card with section")
    ...
    if section_tables:
        ...
        for table in section_tables:
            ...
            table_title, updated_table = extract_title_and_description_for_table(card['section'][0]['content'], table)
            ...
            logger.info(f"Generating visualization for table in section {card['section'][0]['name']}")
            viz_str = generate_visualization(table, user_name, chat_id, report_type=report_type, user_id=user_id)
            if viz_str:
                ...
                logger.info(f"Successfully generated visualization for table {table_id} (type={viz_type})")
    ...
    for idx, sub_section in enumerate(card['sub_sections']):
        ...
        if subsection_tables:
            logger.info(f"Found {len(subsection_tables)} tables in subsection {sub_section['name']}")
            for table in subsection_tables:
                table_title, updated_table = extract_title_and_description_for_table(card['sub_sections'][idx]['content'], table)
                table_id = str(uuid7())
                logger.debug(f"Generated table ID: {table_id}")
                card['sub_sections'][idx]['content'] = replace_table_in_content(
                    card['sub_sections'][idx]['content'],
                    table,
                    updated_table
                )
                table_and_table_id_map[table_id] = table
                logger.info(f"Generating visualization for table in subsection {sub_section['name']}")
                viz_str = generate_visualization(table, user_name, chat_id, report_type=report_type, user_id=user_id)
                if viz_str:
                    # is_mermaid = isinstance(viz_str, str) and viz_str.startswith("mermaid::")
                    is_html = isinstance(viz_str, str) and not viz_str.endswith(".png")
...
def modify_card(card):
    try:
        ...
        if "table_of_contents" in card:
            new_card["section"] = [{"name": "table_of_contents", "content": card["table_of_contents"]}]
            new_card["sub_sections"] = [{"name": "", "content": ""}]
            new_card["citations"] = card["citations"] if "citations" in card else {}
            new_card["summary"] = card["summary"] if "summary" in card else ""
            return new_card
        ...
        if "executive_summary" in card:
            new_card["section"] = [{"name": "executive_summary", "content": card["executive_summary"]}]
            new_card["sub_sections"] = [{"name": "", "content": ""}]
            new_card["citations"] = card["citations"] if "citations" in card else {}
            new_card["summary"] = card["summary"] if "summary" in card else ""
            return new_card
        ...
        logger.error(f"Error converting card format: {str(e)}")
        return {}
...
def modify_report_layout(report_layout):
    try:
        ...
        for item in report_layout:
            ...
            if "title" in item:
                new_card["section"] = [{"name": "title", "content": item["title"], "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                new_card["sub_sections"] = [{"name": "", "content": "", "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                new_card["citations"] = {}
                new_card["summary"] = "" 
            ...
            if "subtitle" in item:
                new_card["section"] = [{"name": "subtitle", "content": item["subtitle"], "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                new_card["sub_sections"] = [{"name": "", "content": "", "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                new_card["citations"] = {}
                new_card["summary"] = ""
                modified_report_layout.append(new_card)
            ...
            if "table_of_contents" in item:
                new_card["section"] = [{"name": "table_of_contents", "content": item["table_of_contents"], "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                new_card["sub_sections"] = [{"name": "", "content": "", "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                new_card["citations"] = {}
                new_card["summary"] = ""
                modified_report_layout.append(new_card)
            ...
            if "executive_summary" in item:
                new_card["section"] = [{"name": "executive_summary", "content": item["executive_summary"], "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                new_card["sub_sections"] = [{"name": "", "content": "", "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                new_card["citations"] = {}
                new_card["summary"] = ""
                modified_report_layout.append(new_card)
            ...
            if "section" in item:
                new_card["section"] = [{"name": item["section"], "content": item.get("content", ""), "tables": [{"visualization": "", "table_id": "", "table_title": ""}]}]
                
                if "sub_sections" in item:
                    formatted_subsections = []
                    for sub in item["sub_sections"]:
                        if isinstance(sub, str):
                            formatted_subsections.append({"name": sub, "content": "", "tables": [{"visualization": "", "table_id": "", "table_title": ""}]})
                        elif isinstance(sub, dict):
                            formatted_subsections.append({"name": sub["name"], "content": sub["content"], "tables": [{"visualization": "", "table_id": "", "table_title": ""}]})
        ...
        logger.error(f"Error modifying report layout: {str(e)}")
        return []
class RefineReportLayout(BaseModel):
    report_layout: str = Field(..., description="The refined and updated report layout in the exact same format as the input, with all sections enhanced using current internet-sourced information while preserving the original structure.")

def refine_report_layout(report_layout, max_retries=3, retry_delay=5, chat_id: str = None, user_id: str = None):
    try:
        # Try OpenAI first
        current_date = datetime.datetime.now().strftime('%B %d, %Y')
        ...
        response = SYNC_OPENAI_CLIENT.responses.parse(
                ...
                input=[
                    {"role": "system", "content": f"You are a precise researcher and technical writer. Use web search always(VERY VERY VERY IMPORTANT) for facts and statistics and retrieve the latest information as per the {current_date}"},
                    {"role": "user", "content": refined_prompt},
                ],
                tools=[{"type": "web_search"}],
                tool_choice={"type": "web_search"},
        ...
        save_raw_llm_response(response, REFINE_REPORT_LAYOUT_MODEL, "Refining the report structure", chat_id, user_id=user_id)
        output = response.model_dump()['output'][-1]
        
        json_output = output['content'][0]['parsed']
        logger.info(f"successfully refined report layout with OpenAI")
        ...
        while retry_count < max_retries:
            try:
                ...
                save_raw_llm_response(interaction, GEMINI_ES_MODEL_ID, "Refining the report structure (backup)", chat_id, user_id=user_id)
                result = json.loads(strip_json_code_fence(interaction.output_text))
                logger.info(f"successfully refined report layout with Gemini (single Interactions API call)")
...
def remove_citations_from_RL(report_layout: str):
    ...
    try:
        ...
        logger.error(f"Error removing citations from RL: {str(e)}")
        return report_layout
...
def remove_citations_from_DRL(DRL):
    ...
    Examples removed:
      ...
      - [1], [1,2], [1-3], [1–3], Arc[2]
    """
    logger.info(f"Removing citations from DRL")
    try:
            ...
            if 'sub_sections' in item:
                for sub_idx,sub_section in enumerate(item['sub_sections']):
                    ...
                    if 'description' in sub_section:
                        sub_section['description'] = remove_citations_from_RL(sub_section['description'])
        logger.info(f"Removed citations from DRL")
        return DRL
    except Exception as e:
        logger.error(f"Error removing citations from DRL: {str(e)}")
        return DRL
...
def standardize_url_citations(text, url_map):
    ...
    try:
        ...
        logger.info(f"URL citation standardization completed. Processed {len(url_to_number)} unique URLs")
        return updated_text, updated_url_map
        
    except Exception as e:
        logger.error(f"Error in standardize_url_citations: {str(e)}")
        return text, url_map
...
def clean_executive_summary(executive_summary):
    ...
    # - Arc[2]
    # - [1][3][4][5][6][7][8][9][10]"""
    logger.info(f"Cleaning executive summary")
    try:
        cleaned_executive_summary = re.sub(r'\[(\d+)\]', '', executive_summary)
        logger.info(f"Successfully cleaned executive summary")
        return cleaned_executive_summary
    except Exception as e:
        logger.error(f"Error cleaning executive summary: {str(e)}")
