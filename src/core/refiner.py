...
import requests
from src.config.constants import OPENAI_API_KEY, OPENAI_LLM, PERPLEXITY_API_KEY, REFINER_MODEL, S3_REPORTS_BASE_PATH, GEMINI_API_KEY, GEMINI_ES_MODEL_ID
from src.core.grep_agent_2 import ask_pdfs as grep_ask_pdfs, PdfSession
from src.core.card_utils import extract_markdown_tables, extract_title_and_description_for_table, generate_section_summary, clean_url, update_summaries_for_ask_caspr, replace_table_in_content, _is_small_table, _parse_table_to_dataframe, fix_numeric_url_citations, _enforce_numbered_citations, repair_malformed_citation_markdown
from src.core.citation_utils import generate_citation_url
from src.core.card_fixer import fix_content
...
from src.config.constants import VISUALIZATION_MODEL
...
from src.core.napkin_ai import make_napkin_ai_visual_request
from src.core.prompt_utils import NAPKIN_AI_TABLE_TO_VIZ_PROMPT, PANDASAI_VIZ_REFINE_PROMPT, REFINE_SCHEMA_GEMINI, TABLE_TO_VIZ_PROMPT, PANDASAI_VIZ_REFINE_PROMPT, REFINE_PROMPT_WITHOUT_UPLOAD, REFINE_PROMPT_WITH_UPLOAD
from src.core.s3_utils import get_s3_instance
from src.config.log_helper import setup_logging
from src.config.constants import ENVIRONMENT, SYNC_OPENAI_CLIENT
from src.core.llm_response_logger import save_raw_llm_response, strip_json_code_fence
from src.core.refine_table_policy import (
    classify_visualization_intent,
    process_refined_tables,
)
from src.core.refine_visualizer.viz_refine import (
    ...
    for idx, card in enumerate(cards_for_db):
        for section in card.get('section', []):
            if section.get('name') == targeted_section_or_subsection:
                refine_target = {"name": section['name'], "content": section['content'], "idx": idx,"sub_idx": None}
                break
        for sub_idx, subsection in enumerate(card.get('sub_sections', [])):
            if subsection.get('name') == targeted_section_or_subsection:
                refine_target = {"name": subsection['name'], "content": subsection['content'], "idx": idx, "sub_idx": sub_idx}
                break
...
def generate_refined_content(user_prompt, 
    refine_target, 
    max_retries=3, 
    retry_delay=5, 
    refinement_history=None,
    file_id=None,
    upload_file_config: Optional[Dict[str, Any]] = None,
    user_id=None,
    web_search=True,
    ...
    if refinement_history and len(refinement_history) > 0:
        ...
        for idx, history_item in enumerate(refinement_history, 1):
            section_name = history_item['name_of_the_section_or_subsection_that_user_previously_refined']
    ...
    if grep_session is not None:
        logger.info(f"Refining with grep_agent_2 tool (grep_session provided) | has_report_file={bool(file_id)}")
        ...
        try:
            ...
            for _round in range(max_tool_rounds):
                ...
                save_raw_llm_response(response, model, "Refining a report card using document search", chat_id, user_id=user_id)
            ...
            logger.error(f"Error with OpenAI API (grep-based refinement): {str(e)}")
        ...
        while gemini_retry_count < max_retries:
            try:
                logger.info(f"Falling back to Gemini API for grep-based refinement (attempt {gemini_retry_count + 1}/{max_retries})")
                ...
                for _round in range(max_tool_rounds):
                    ...
                    fn_calls = [step for step in interaction.steps if step.type == "function_call" and step.name == "search_documents"]
                    if not fn_calls:
                        break

                    function_results = []
                    for fc in fn_calls:
                        fn_args = fc.arguments if isinstance(fc.arguments, dict) else json.loads(fc.arguments)
                        result_text = _grep_tool_handler(fn_args)
                    ...
                    logger.info(f"[grep_agent_2][Gemini] Round {_round + 1} — resolved {len(fn_calls)} search_documents call(s)")
                    gemini_input = function_results
                    previous_interaction_id = interaction.id

                save_raw_llm_response(
                    final_interaction, GEMINI_ES_MODEL_ID, "Refining a report card using document search (backup)",
                    chat_id, user_id=user_id,
                )

                gemini_citations = _extract_gemini_refine_citations(final_interaction, final_interaction.output_text)
                return final_interaction.output_text, gemini_citations, {}
            except Exception as e:
                gemini_retry_count += 1
                logger.warning(f"Gemini API attempt {gemini_retry_count}/{max_retries} failed (grep-based refinement): {str(e)}")
                if gemini_retry_count < max_retries:
                    logger.info(f"Retrying Gemini in {retry_delay} seconds...")
                    time.sleep(retry_delay)
                else:
                    logger.critical(f"All Gemini API retry attempts failed (grep-based refinement): {str(e)}")
                    return None, None, {}
    ...
    while retry_count < max_retries:
        try:
            logger.info(f"Trying OpenAI API for refinement (attempt {retry_count + 1}/{max_retries})")
            ...
            if upload_file_config is not None:
                ...
                if vector_store_id and file_ids:
                    ...
                    tools = [
                        {"type": "web_search"},
                        {"type": "file_search", "vector_store_ids": [vector_store_id], "filters": filters}
                    ]
            if file_id:
            ...
            if web_search and not vector_store_id:
                ...
                save_raw_llm_response(response, model, "Refining a report card using web research", chat_id, user_id=user_id)
                ...
                save_raw_llm_response(response, model, "Refining a report card from uploaded documents", chat_id, user_id=user_id)
                ...
                save_raw_llm_response(response, model, "Refining a report card using documents and web research", chat_id, user_id=user_id)
            else:
                ...
                save_raw_llm_response(response, model, "Refining a report card", chat_id, user_id=user_id)
            ...
            if retry_count < max_retries:
                ...
                logger.critical(f"All OpenAI API retry attempts failed: {str(e)}")
                break 
    ...
    while retry_count < max_retries:
        try:
            ...
            save_raw_llm_response(interaction, GEMINI_ES_MODEL_ID, "Refining a report card (backup)", chat_id, user_id=user_id)
...
def replace_citations(cards_for_db, content, citation_urls, url_to_snippet: Optional[Dict[str, str]] = None):
    ...
    try:
        ...
        def replacer(match):
            ...
            if 0 <= index < len(citation_urls):
                ...
                if url not in citation_dict:
                    citation_dict[url] = next_citation_number
                    logger.debug(f"Numbered citation - new url assigned: {url} -> {next_citation_number}")
                    next_citation_number += 1
                
                citation_number = citation_dict[url]
                logger.debug(f"Numbered citation - using citation_number: {citation_number} for url: {url}")
        ...
        def openai_replacer(match):
            ...
            citation_number = citation_dict[url]
            logger.debug(f"OpenAI citation - using citation_number: {citation_number} for url: {url}")
            final_url = _build_citation_url(url)
            return f"[{citation_number}]({final_url})"
        ...
        def standard_openai_replacer(match):
            ...
            if url not in citation_dict:
                citation_dict[url] = next_citation_number
                logger.debug(f"Standard OpenAI citation - new url assigned: {url} -> {next_citation_number}")
                next_citation_number += 1

            citation_number = citation_dict[url]
            logger.debug(f"Standard OpenAI citation - using citation_number: {citation_number} for url: {url}")
            final_url = _build_citation_url(url)
            return f"[{citation_number}]({final_url})"
        ...
        def domain_citation_replacer(match):
            ...
            citation_number = citation_dict[url]
            logger.debug(f"Domain citation - using citation_number: {citation_number} for url: {url}")
            final_url = _build_citation_url(url)
            return f"[{citation_number}]({final_url})"
        ...
        def descriptive_citation_replacer(match):
            ...
            if url_match:
                ...
                if url not in citation_dict:
                    citation_dict[url] = next_citation_number
                    logger.debug(f"Descriptive citation - new url assigned: {url} -> {next_citation_number}")
                    next_citation_number += 1

                citation_number = citation_dict[url]
                logger.debug(f"Descriptive citation - using citation_number: {citation_number} for url: {url}")
                final_url = _build_citation_url(url)
                return f"[{citation_number}]({final_url})"
            else:
        ...
        updated_content = re.sub(descriptive_citation_pattern, descriptive_citation_replacer, updated_content)
        ...
        def markdown_link_replacer(match):
            ...
            if re.search(r'[a-zA-Z0-9-]+\.[a-zA-Z]{2,}', url):
                ...
                if url in citation_dict:
                    citation_number = citation_dict[url]
                    logger.info(f"Found existing citation: [{link_text}] -> [{citation_number}]({url})")
                else:
                    # Assign new citation number
                    citation_dict[url] = next_citation_number
                    citation_number = next_citation_number
                    logger.info(f"Created new citation: [{link_text}] -> [{citation_number}]({url})")
                    next_citation_number += 1
        ...
        def malformed_citation_replacer(match):
            ...
            if url_match:
                ...
                if url in citation_dict:
                    citation_number = citation_dict[url]
                    logger.info(f"Found existing citation for malformed URL: [{citation_number}]({url})")
                else:
                    # Assign new citation number
                    citation_dict[url] = next_citation_number
                    citation_number = next_citation_number
                    logger.info(f"Created new citation for malformed URL: [{citation_number}]({url})")
                    next_citation_number += 1
        ...
        logger.info(f"Citations replacement completed successfully")
...
def id_to_target(cards_for_db, fe_json_for_refine)->Tuple[str, str]:
    ...
    if "subsection" in fe_json_for_refine and fe_json_for_refine["subsection"] is not None:
        ...
        if "id" in subsection and "refine_or_delete_prompt" in subsection and subsection["refine_or_delete_prompt"] is not None:
            id = subsection["id"]
...
def generate_refine_table_visualization(table: str, chat_id: str, aspect_ratio: Optional[str] = "4:3", max_retries: int = 6, report_type: str = "study", user_id: Optional[str] = None)-> str:
    """Generate refine table visualization with decision-based HTML first, then Gemini/PandasAI fallback.
    ...
    logger.info(f"Generating refine table visualization for table")
    ...
    try:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                logger.info(f"Gemini image gen attempt {attempt}/{max_retries} using GEMINI_API_KEY_{api_key_num}")
                ...
                save_raw_llm_response(interaction, VISUALIZATION_MODEL, "Updating a chart image after card edits", chat_id, user_id=user_id)
                ...
                logger.error(f"Gemini attempt {attempt}/{max_retries} (GEMINI_API_KEY_{api_key_num}) failed: {e}")
    ...
    try:
        ...
        dest_dir = f"temp_viz_dir_{chat_id}"
...
def generate_visualization(table: str, user_name: str, chat_id: str, report_type: str = "study", user_id: Optional[str] = None):
    """
    ...
    try:
        ...
        image_path = generate_refine_table_visualization(table, chat_id, report_type=report_type, user_id=user_id)
...
def refine_card(cards_for_db, 
    ...
    if target is None:
        logger.error(f"Target section '{target_section}' not found in cards_for_db. Card ID may be invalid.")
    ...
    refined_content, citations, url_to_snippet = generate_refined_content(
        refine_prompt, original_content,
        refinement_history=refinement_history, file_id=file_id,
        upload_file_config=upload_file_config, user_id=_uid,
        web_search=web_search, grep_session=grep_session, chat_id=chat_id,
    ...
    for i, history_item in enumerate(refinement_history):
        if history_item['name_of_the_section_or_subsection_that_user_previously_refined'] == target['name']:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False, encoding='utf-8') as tmp_file:
            json.dump({"section": [{"name": "Refined_version", "content": refined_content},]}, tmp_file, indent=2, ensure_ascii=False)
    ...
    updated_content, citations_dict = replace_citations(cards_for_db, refined_content, citations, url_to_snippet)
    updated_content = _enforce_numbered_citations(updated_content, complete_report_json=cards_for_db)
    updated_card_table_map = {}
    if target["sub_idx"] is None:
            ...
            for index, content in enumerate(cards_for_db[target["idx"]]["section"]):
                if content["name"] == target["name"]:
                    table_result = process_refined_tables(
                        ...
                        force_generate=lambda table, prompt, existing_viz: generate_forced_visualization(
                            table,
                            prompt,
                            user_name,
                            chat_id,
    ...
    try:
        for index,item in enumerate(updated_card["section"]):
            try:
                ...
                logger.error(f"Error generating visualization for refined section {item['name']}: {e}")
        for index,item in enumerate(updated_card["sub_sections"]):
            try:
                ...
                logger.error(f"Error generating visualization for refined sub_section {item['name']}: {e}")
    except Exception as e:
