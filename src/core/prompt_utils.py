...
PANDASAI_VIZ_REFINE_PROMPT = """Create a visually stunning and aesthetically refined plot based on the provided data {table}. 
            Ensure the visualization is accurate, complete, and robust. Use a clean layout with non-overlapping text 
            and axis labels. Use ONLY the following Caspr color palette — no other colors allowed:
            - #E8453C (red) for highlights and key insights
            - #0B0B09 (black) for primary text and typography
            - #1A1A18 (dark grey) for secondary elements
            - #6B6B66 (mid grey) for supporting data and labels
            - #F2F1EF (light grey) for backgrounds and fills
            - #D6D4CF for borders and grid lines
            - #FFFFFF (white) for plot background
            Include appropriate legends only where necessary to enhance clarity without clutter. 
            The plot should be intuitive and easy to interpret, even for non-technical audiences. 
            Set figure size to (8, 6) inches for optimal display. Avoid using plt.show()"""
