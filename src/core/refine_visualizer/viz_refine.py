...
5. Fallback -> PandasAI (table + user_ins only, NOT existing viz)
"""
import base64
import datetime
import os
...
from src.config.constants import OPENAI_LLM, S3_REPORTS_BASE_PATH
from src.core.refine_visualizer.html_graph_maker_refine import generate_html_visualization_refine
from src.core.refine_visualizer.html_table_visualizer_refine import generate_info_visualization_refine
from src.core.dynamic_prompting import decide_if_table_needs_visualization
from src.core.s3_utils import get_s3_instance

from src.core.prompt_utils import PANDASAI_VIZ_REFINE_PROMPT, TABLE_TO_VIZ_PROMPT
...
def generate_refine_table_visualization(table: str, 
    ...
    """Generate refined table visualization with decision-based HTML first, then Gemini/PandasAI fallback.
    ...
    Flow:
        ...
        5. If Gemini fails -> PandasAI fallback (table + user_ins only, NOT viz)
    ...
    formatted_viz_prompt = PANDASAI_VIZ_REFINE_PROMPT.format(table=table, user_prompt=user_prompt) + f" Make it in English only {user_prompt}(ENGLISH ONLY)" if user_prompt else PANDASAI_VIZ_REFINE_PROMPT.format(table=table, user_prompt="")
    last_error = None
    try:
        # raise Exception("test")
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                if attempt == max_retries:
                    logger.warning(f"All {max_retries} Gemini API attempts failed, falling back to PandasAI")
                    raise last_error
                continue
        ...
        # Step 4: Fall back to PandasAI (table + user_ins only, NOT existing viz)
        logger.info(f"Falling back to PandasAI for visualization (table + user_ins only)")
        dest_dir = "temp_viz_dir"                                                                  
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error parsing HTML table content: {str(html_error)}")
                logger.error(f"HTML content that failed: {html_content}")
                return None
            ...
            pandas_max_retries = 5
            retry_count = 0
            
            while retry_count < pandas_max_retries:
                try:
                    ...
                    retry_count += 1
                    logger.warning(f"Attempt {retry_count}/{pandas_max_retries} failed: {str(e)}")
                    if retry_count >= pandas_max_retries:
                        logger.error(f"All {pandas_max_retries} attempts failed")
                        raise
                    time.sleep(1)
            ...
            if new_images:
                image = new_images[0]
                image_path = os.path.join(dest_dir, image)
            if image_path:
                return image_path
                
        except Exception as e:
            logger.error(f"Error generating visualization with PandasAI: {str(e)}")
            return None
