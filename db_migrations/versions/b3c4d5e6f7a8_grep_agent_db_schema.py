...
def upgrade():
    ...
    op.add_column(
        "uploaded_files",
        sa.Column(
            "parsed_at", sa.DateTime(timezone=True), nullable=True, comment="When parsing completed"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "text", sa.Text(), nullable=False, comment="~800 char chunk text extracted from the PDF"
        ),
        sa.Column(
            "page_num",
            sa.Integer(),
        ...
        sa.Column("end_offset", sa.Integer(), nullable=False, comment="Character offset end"),
        sa.Column(
    ...
    # -------------------------------------------------------------------------
    op.create_index("idx_uploaded_files_is_parsed", "uploaded_files", ["is_parsed"])
    op.create_index("idx_ufc_uploaded_file_id", "uploaded_file_chunks", ["uploaded_file_id"])
    op.create_index("idx_ufc_file_page", "uploaded_file_chunks", ["uploaded_file_id", "page_num"])
