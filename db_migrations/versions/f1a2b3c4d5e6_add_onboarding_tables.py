...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('email_domain', sa.String(255), unique=True, nullable=False, comment='e.g. iitb.ac.in'),
        sa.Column('discount_percentage', sa.Integer(), nullable=False, server_default=sa.text('50')),
        sa.Column('is_active', sa.Boolean(), nullable=False, server_default=sa.text('true')),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    )
    ...
    op.create_table(
        ...
        sa.Column('research_interest_id', sa.CHAR(36), sa.ForeignKey('research_interests.id', ondelete='CASCADE'), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    ...
    op.create_index('idx_user_research_interests_interest_id', 'user_research_interests', ['research_interest_id'])

    # ------------------------------------------------------------------
    # 5. New columns on users
    # ------------------------------------------------------------------
    op.add_column('users', sa.Column('user_role_id', sa.CHAR(36), sa.ForeignKey('user_roles.id', ondelete='SET NULL'), nullable=True, comment='Selected role from onboarding'))
    op.add_column('users', sa.Column('university_email', sa.String(255), nullable=True, comment='University email for student verification'))
    op.add_column('users', sa.Column('is_university_verified', sa.Boolean(), nullable=False, server_default=sa.text('false'), comment='Whether the university email has been verified'))
    op.add_column('users', sa.Column('university_id', sa.CHAR(36), sa.ForeignKey('universities.id', ondelete='SET NULL'), nullable=True, comment='Partner university after domain validation'))
    op.add_column('users', sa.Column('onboarding_completed', sa.Boolean(), nullable=False, server_default=sa.text('false'), comment='Whether the user finished the onboarding flow'))
    ...
    # ------------------------------------------------------------------
    roles = [
        ('founder_entrepreneur', 'Founder / Entrepreneur', 'Validating ideas and understanding markets', None, 1),
        ('consultant', 'Consultant', 'Creating insights and reports for clients', None, 2),
        ('investor_vc_analyst', 'Investor / VC / Analyst', 'Evaluating markets, startups, and industries', None, 3),
        ('marketing_growth', 'Marketing / Growth', 'Understanding customers and market trends', None, 4),
        ('market_researcher_analyst', 'Market Researcher / Analyst', 'Running research and producing insights', None, 5),
        ('corporate_strategy_business_ops', 'Corporate Strategy / Business Ops', 'Strategic planning and competitive intelligence', None, 6),
        ('student_academic_researcher', 'Student / Academic Researcher', 'Learning and conducting academic research', '50% OFF!', 7),
    ...
    op.bulk_insert(user_roles_table, [
        {'id': str(uuid7()), 'name': r[0], 'display_name': r[1], 'description': r[2], 'discount_tag': r[3], 'display_order': r[4]}
        for r in roles
    ])
    ...
    interests = [
        ...
        ('competitive_analysis', 'Competitive Analysis', 'Analyze competitors, positioning, and strategies', 2),
        ('startup_idea_validation', 'Startup / Idea Validation', 'Test business ideas and opportunities', 3),
    ...
    op.bulk_insert(research_interests_table, [
        {'id': str(uuid7()), 'name': i[0], 'display_name': i[1], 'description': i[2], 'display_order': i[3]}
        for i in interests
    ])
