...
def upgrade() -> None:
    ...
    op.execute(text("UPDATE cards SET updated_at = created_at WHERE updated_at IS NULL"))

    # Step 3: Make column NOT NULL with server default for new inserts
    op.alter_column(
        "cards", "updated_at", nullable=False, server_default=sa.text("CURRENT_TIMESTAMP")
    )
